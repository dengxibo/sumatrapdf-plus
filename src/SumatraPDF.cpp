...
LoadArgs* LoadArgs::Clone() {
    ...
    res->engine = engine;
    res->showWin = showWin;
    res->forceReuse = forceReuse;
    res->noPlaceWindow = noPlaceWindow;
    res->isNewWindow = isNewWindow;
    res->placeWindow = placeWindow;
...
static RectF GetThumbnailCropRect(EngineBase* engine, int pageNo, Size targetSize) {
    RectF pageRect = engine->PageMediabox(pageNo);
    if (pageRect.IsEmpty() || targetSize.dx <= 0 || targetSize.dy <= 0) {
        return pageRect;
    }

    pageRect = engine->Transform(pageRect, pageNo, 1.0f, 0);
    float targetAspect = (float)targetSize.dx / (float)targetSize.dy;
    float pageAspect = pageRect.dx / pageRect.dy;
    if (pageAspect > targetAspect) {
        float cropDx = pageRect.dy * targetAspect;
        pageRect.x += (pageRect.dx - cropDx) / 2.f;
        pageRect.dx = cropDx;
    } else {
        float cropDy = pageRect.dx / targetAspect;
        pageRect.y += (pageRect.dy - cropDy) / 2.f;
        pageRect.dy = cropDy;
    }
    return engine->Transform(pageRect, pageNo, 1.0f, 0, true);
}

void ControllerCallbackHandler::RenderThumbnail(DisplayModel* dm, Size size, const OnBitmapRendered* saveThumbnail) {
    auto engine = dm->GetEngine();
    RectF pageRect = GetThumbnailCropRect(engine, 1, size);
    if (pageRect.IsEmpty()) {
...
static void CreateThumbnailFromFileThread(CreateThumbnailFromFileData* d) {
    SetCreateEngineForThumbnail(true);
    defer {
        SetCreateEngineForThumbnail(false);
    };
    ...
    RectF pageRect = GetThumbnailCropRect(engine, 1, Size{kThumbnailDx, kThumbnailDy});
    if (pageRect.IsEmpty()) {
        engine->Release();
        delete d;
        return;
...
static bool WinCtrlIsOwnedByAnyTab(MainWindow* win, DocController* ctrl) {
    if (!ctrl || !win) {
        return false;
    }
    for (auto& t : win->Tabs()) {
...
static WindowTab* TabForWinCtrl(MainWindow* win, DocController* ctrl) {
    if (!ctrl || !win) {
        return nullptr;
    }
    for (auto& t : win->Tabs()) {
...
static void DetachWinCtrlIfMatches(MainWindow* win, DocController* ctrl) {
    if (win && ctrl && win->ctrl == ctrl) {
        win->ctrl = nullptr;
    }
}
...
static void ReplaceDocumentInCurrentTab(LoadArgs* args, DocController* ctrl, FileState* fs) {
    ...
    if (win->ctrl) {
        ...
        if (dm) {
            ...
            // clamp saved page only after all pages are known; during progressive
            // reflow loading DisplayModel defers restore until pages are available
            if (win->ctrl->PageCount() > 0 && !EngineIsProgressiveEbookLoading(engine)) {
                int maxPage = win->ctrl->PageCount();
                if (ss.page > maxPage) {
                    ss.page = maxPage;
                    ss.x = -1;
                    ss.y = -1;
    ...
    if (fs) {
        ...
        } else if (win->ctrl->PageCount() > 0 && !EngineIsProgressiveEbookLoading(tab->GetEngine())) {
            ss.page = limitValue(ss.page, 1, win->ctrl->PageCount());
        }
...
void ShowMainWindow(MainWindow* win, int windowState) {
    // Suppress an intermediate paint with stale find-control positions (issue #5456).
    SendMessageW(win->hwndFrame, WM_SETREDRAW, FALSE, 0);
...
// progressive ebook early display can attach a DisplayModel before
// LoadDocumentFinish runs SetInitialViewSettings (e.g. background tab)
static void EnsureDisplayModelPagesInfo(MainWindow* win, WindowTab* tab) {
    DisplayModel* dm = tab ? tab->AsFixed() : nullptr;
    if (!dm) {
        return;
    }
    bool needPagesInfo = !dm->pagesInfo;
    bool needZoom = !IsValidZoom(dm->zoomVirtual);
    ...
    int rotation = 0;
    if (tab->tabState) {
        displayMode = DisplayModeFromString(tab->tabState->displayMode, DisplayMode::Automatic);
        startPage = tab->tabState->pageNo;
        zoomVirtual = ZoomFromString(tab->tabState->zoom, zoomVirtual);
        rotation = tab->tabState->rotation;
    } else if (gGlobalPrefs->rememberStatePerDocument && tab->filePath) {
...
// (LoadDocumentFinish assumes CurrentTab() matches the tab bar selection)
static void AttachDocumentToBackgroundTab(LoadArgs* args, WindowTab* tab) {
    MainWindow* win = args->win;
    ...
    // (the foreground tab), not tab->ctrl, and crashes during concurrent loads.
    if (tab->tabState) {
    ...
    if (gGlobalPrefs->reloadModifiedDocuments) {
        ...
        tab->watcher = FileWatcherSubscribe(path, fn, enableManualCheck);
    }
...
static void PrepareLoadingTab(MainWindow* win, LoadArgs* args) {
    if (!win || !args) {
        return;
    }
    const char* fullPath = args->FilePath();
    ...
    if (!IsWindowVisible(win->hwndFrame) && args->showWin) {
...
// thread_local: multiple RunAsync load threads can overlap (e.g. restore tabs);
// a single global was overwritten and paired the wrong engine with LoadArgs.
static thread_local LoadDocumentAsyncData* gCurrentAsyncLoad = nullptr;

struct EarlyEngineDisplayTask {
    EngineBase* engine = nullptr;
...
static void EarlyEngineDisplayUI(EarlyEngineDisplayTask* task) {
    ...
    if (task->engine->pageCount <= 0) {
        return;
    }

    DisplayModel* dm = new DisplayModel(task->engine, win->cbHandler);
    VerifyController(dm, args->FilePath());
    gMostRecentlyOpenedDoc = dm;
    args->ctrl = dm;
    d->engine = nullptr;
...
void NotifyEngineDisplayReady(EngineBase* engine) {
    if (!gCurrentAsyncLoad || !engine) {
        return;
    }
    const char* enginePath = engine->FilePath();
    const char* loadPath = gCurrentAsyncLoad->args->FilePath();
    if (!str::Eq(enginePath, loadPath)) {
        return;
    }
    auto* task = new EarlyEngineDisplayTask();
    task->engine = engine;
...
static void LoadDocumentAsyncFinish(LoadDocumentAsyncData* d) {
    ...
    const char* path = args->FilePath();
    if (!args->ctrl && d->engine && d->engine->pageCount > 0) {
        args->ctrl = new DisplayModel(d->engine, win->cbHandler);
...
static void LoadDocumentAsync(LoadDocumentAsyncData* d) {
    ...
    gCurrentAsyncLoad = d;
    InterlockedExchange(&d->displayedOnUI, 0);
    ...
    d->engine = engine;
    if (!engine) {
        args->ctrl = CreateControllerForEngineOrFile(nullptr, path, &pwdUI, win);
        d->engine = nullptr;
    } else if (engine->pageCount <= 0) {
        SafeEngineRelease(&engine);
        d->engine = nullptr;
        args->ctrl = nullptr;
    } else {
...
MainWindow* LoadDocument(LoadArgs* args) {
    ...
    if (ShouldLoadDocumentAsync(path, args)) {
        StartLoadDocument(args);
        return args->win;
    }
...
static void CloseDocumentInCurrentTab(MainWindow* win, bool keepUIEnabled, bool deleteModel) {
    ...
    WindowTab* unloadingTab = TabForWinCtrl(win, win->ctrl);
    if (!unloadingTab) {
    ...
    if (unloadingTab) {
        ...
        if (deleteModel) {
            ...
            // Tab switch: pause and remember position instead of clearing read-aloud state.
...
void CloseWindow(MainWindow* win, bool quitIfLast, bool forceClose) {
    ...
    // quitIfLast=true (user clicked X / File->Exit) must also proceed so a
    // prior in-progress close cannot block application shutdown.
    if (win->isBeingClosed && !forceClose && !quitIfLast) {
        return;
    }
...
// true if the window's native vertical scrollbar is currently shown (not
// tracked separately from the scrollbar being present-but-invisible)
static bool WindowHasVisibleVScrollbar(HWND hwnd) {
    if (!hwnd || !IsWindow(hwnd)) {
        return false;
    }
    SCROLLBARINFO sbi{};
...
static void RelayoutFrame(MainWindow* win, bool updateToolbars, int sidebarDx) {
    ...
    // Never copy old client pixels to the new geometry. In particular, the
    // TreeView's non-client scrollbar and the splitter line otherwise get
    // copied to every intermediate x position and remain as vertical trails.
    ...
    if (tocVisible || favVisible) {
        ...
        if (tocVisible) {
            ...
            if (favVisible) {
                Rect rSplitV(rc.x, rc.y + toc.dy, toc.dx, splitterDy);
        ...
        Rect rSplitH(rc.x + toc.dx, rc.y, splitterDx, rc.dy);
        dh.MoveWindow(win->sidebarSplitter->hwnd, rSplitH, livePosFlags);

        rc.x += toc.dx + splitterDx;
    ...
    if (liveSidebarDrag) {
        // Geometry is final. The CopyBits moves above already shifted each
        // window's on-screen pixels and the system invalidated the exposed
        // strips. This pass (a) erase-invalidates the containers so vacated
        // strips (e.g. where a scrollbar used to be) repaint with the sidebar
        // background and (b) re-arms the trees' native non-client scrollbar
        // frame at its new edge, which child invalidation does not cover. No
        // RDW_UPDATENOW: Windows merges these update regions between mouse
        // moves and paints through the normal WM_PAINT / WM_NCPAINT cycle.
        if (tocVisible) {
            RedrawWindow(win->hwndTocBox, nullptr, nullptr, RDW_ERASE | RDW_INVALIDATE | RDW_ALLCHILDREN);
            if (win->tocTreeView) {
                RedrawWindow(win->tocTreeView->hwnd, nullptr, nullptr, RDW_INVALIDATE | RDW_FRAME);
            }
        }
        if (favVisible) {
            ...
            if (win->favTreeView && win->favTreeView->hwnd) {
                RedrawWindow(win->favTreeView->hwnd, nullptr, nullptr, RDW_INVALIDATE | RDW_FRAME);
            }
        }
        // The canvas repaints through its own WM_SIZE -> UpdateCanvasSize ->
        // InvalidateRect pipeline (coalesced by the message loop), just like
        // a main-window resize; no forced repaint per mouse move.
    } else if (tocVisible) {
        RedrawWindow(win->hwndTocBox, nullptr, nullptr, RDW_ERASE | RDW_INVALIDATE | RDW_ALLCHILDREN);
    }
    ...
    if (updateToolbars && win->isToolbarVisible && win->hwndToolbar) {
...
struct TocDragPerf {
    int moves = 0;
    double relayoutMs = 0;
    double maxRelayoutMs = 0;
};
static TocDragPerf gTocDragPerf;
static int gTocDragStartDx = -1;
static int gTocDragLastDx = -1;

static bool TocDragLogEnabled() {
    static int cached = -1;
    if (cached < 0) {
        cached = (GetEnvironmentVariableA("SUMATRA_TOC_DRAG_LOG", nullptr, 0) > 0 ||
                  GetEnvironmentVariableA("SUMATRA_TOC_DRAG_BENCH", nullptr, 0) > 0)
                     ? 1
                     : 0;
    }
    return cached != 0;
...
static double TocDragQpcMs(LARGE_INTEGER from, LARGE_INTEGER to) {
    LARGE_INTEGER freq;
    QueryPerformanceFrequency(&freq);
    return (double)(to.QuadPart - from.QuadPart) * 1000.0 / (double)freq.QuadPart;
}

static void TocDragLogSummary(int dxStart, int dxEnd, bool sbarVisible) {
    if (!TocDragLogEnabled() || gTocDragPerf.moves == 0) {
        return;
    }
    int n = gTocDragPerf.moves;
    logfa("TOC-DRAG moves=%d dx=%d->%d sbar=%d relayout avg=%.2fms max=%.2fms", n, dxStart, dxEnd, sbarVisible ? 1 : 0,
          gTocDragPerf.relayoutMs / n, gTocDragPerf.maxRelayoutMs);
}

// Mouse input can arrive much faster than a full document relayout and paint.
// Keep only the newest requested width and let the message queue perform at
// most one resize per pump iteration. This mirrors upstream's
// ScheduleUiUpdate() splitter path without pulling in its larger UI-state
// refactor.
static void ScheduleSidebarRelayout(MainWindow* win, int sidebarDx) {
    win->pendingSidebarDx = sidebarDx;
    if (win->sidebarRelayoutPending) {
        return;
    }
...
static void RunScheduledSidebarRelayout(MainWindow* win) {
    if (!win || !win->sidebarRelayoutPending) {
        return;
    }
    int sidebarDx = win->pendingSidebarDx;
    win->sidebarRelayoutPending = false;
    win->pendingSidebarDx = -1;
...
static void OnSidebarSplitterMove(Splitter::MoveEvent* ev) {
    ...
    if (gSidebarSplitterWrapSuspended) {
        ...
        win->sidebarRelayoutPending = false;
        win->pendingSidebarDx = -1;
        bool sbarWasVisible = win->tocTreeView && WindowHasVisibleVScrollbar(win->tocTreeView->hwnd);
        TocDragLogSummary(gTocDragStartDx, gTocDragLastDx, sbarWasVisible);
        // Final pass keeps live-drag semantics: RelayoutFrame must not send
        // WM_SETREDRAW to the frame, because DefWindowProc implements that
        // message on a top-level window by clearing WS_VISIBLE, which makes
        // Explorer drop the taskbar button (~1s) even though the window keeps
        // painting on screen.
...
static void OnFavSplitterMove(Splitter::MoveEvent* ev) {
    ...
    if (gSidebarSplitterWrapSuspended) {
        // Same as the sidebar splitter: keep live-drag semantics for the final
        // pass so the frame never receives WM_SETREDRAW (it clears WS_VISIBLE
        // on a top-level window and makes Explorer drop the taskbar button).
        RelayoutFrame(win, false, rToc.dx);
        gSidebarSplitterWrapSuspended = false;
        UpdateOverlayScrollbarPositions(win);
        win->UpdateCanvasSize();
...
// Synthetic splitter-drag benchmark (SUMATRA_TOC_DRAG_BENCH="totalPx,stepPx";
// implies logging): drives the exact live-drag code path (RelayoutFrame) with
// synthetic widths once after a document's TOC loads, to A/B measure
// scrollbar vs. no-scrollbar cost without real mouse input.
static void MaybeRunTocDragBench(MainWindow* win) {
    static bool done = false;
    if (done || !win || !win->tocLoaded || !win->tocVisible || !win->tocTreeView) {
        return;
    }
    char buf[64] = {0};
    if (GetEnvironmentVariableA("SUMATRA_TOC_DRAG_BENCH", buf, dimof(buf)) == 0) {
        return;
    }
    done = true;
    int totalPx = atoi(buf);
    char* comma = strchr(buf, ',');
    int stepPx = comma ? atoi(comma + 1) : 8;
    if (totalPx <= 0 || stepPx <= 0) {
        return;
    }
    char expandBuf[8] = {0};
    bool expandAll = GetEnvironmentVariableA("SUMATRA_TOC_DRAG_EXPAND", expandBuf, dimof(expandBuf)) > 0;
    if (expandAll) {
    ...
    for (int pass = 0; pass < 2; pass++) {
        // pass 0: grow by totalPx, pass 1: shrink back
        int dir = (pass == 0) ? 1 : -1;
        int dx0 = WindowRect(win->hwndTocBox).dx;
        gTocDragPerf = {};
        gTocDragStartDx = dx0;
        gTocDragLastDx = dx0;
        SuspendTreeWrapLiveResizeForWindow(win);
        gSidebarSplitterWrapSuspended = true;
        int target = dx0 + dir * totalPx;
        int dx = dx0;
        while (true) {
            int next = dx + dir * stepPx;
            bool last = (dir > 0) ? (next >= target) : (next <= target);
            if (last) {
                next = target;
            }
            dx = next;
            ...
            gTocDragPerf.moves++;
            double ms = TocDragQpcMs(t0, t1);
            gTocDragPerf.relayoutMs += ms;
            if (ms > gTocDragPerf.maxRelayoutMs) {
                gTocDragPerf.maxRelayoutMs = ms;
            }
            gTocDragLastDx = dx;
            // pump pending messages between synthetic moves so WM_PAINT /
            // WM_SIZE are processed like during a real drag
            MSG benchMsg;
            while (PeekMessageW(&benchMsg, nullptr, 0, 0, PM_REMOVE)) {
                TranslateMessage(&benchMsg);
                DispatchMessageW(&benchMsg);
            }
            if (last) {
                break;
            }
        }
        bool sbar = WindowHasVisibleVScrollbar(win->tocTreeView->hwnd);
        TocDragLogSummary(gTocDragStartDx, gTocDragLastDx, sbar);
        // finalize exactly like a real drag end
        gSidebarSplitterWrapSuspended = false;
        RelayoutFrame(win, false, dx);
        win->UpdateCanvasSize();
        ResumeTreeWrapLiveResizeAndFlush(win);
...
static void LaunchBrowserWithSelection(WindowTab* tab, const char* urlPattern) {
    ...
    LaunchBrowserWithReuse(uri);
}

enum class DoubaoPromptKind {
    Word,
    Sentence,
    Chinese,
};

static bool Utf8DecodeOne(const char*& s, char32_t* cpOut) {
    if (!s || !*s || !cpOut) {
        return false;
    }
    u8 c0 = (u8)s[0];
    ...
    if ((c0 & 0xE0) == 0xC0 && s[1]) {
        *cpOut = ((c0 & 0x1F) << 6) | ((u8)s[1] & 0x3F);
        s += 2;
        return true;
    }
    if ((c0 & 0xF0) == 0xE0 && s[1] && s[2]) {
        *cpOut = ((c0 & 0x0F) << 12) | (((u8)s[1] & 0x3F) << 6) | ((u8)s[2] & 0x3F);
        s += 3;
        return true;
    }
    if ((c0 & 0xF8) == 0xF0 && s[1] && s[2] && s[3]) {
        *cpOut = ((c0 & 0x07) << 18) | (((u8)s[1] & 0x3F) << 12) | (((u8)s[2] & 0x3F) << 6) | ((u8)s[3] & 0x3F);
        s += 4;
        return true;
    }
    *cpOut = c0;
...
static bool IsCjkIdeograph(char32_t cp) {
    return (cp >= 0x4E00 && cp <= 0x9FFF) || (cp >= 0x3400 && cp <= 0x4DBF) || (cp >= 0xF900 && cp <= 0xFAFF);
}

static bool IsLatinLetter(char32_t cp) {
    return (cp >= 'A' && cp <= 'Z') || (cp >= 'a' && cp <= 'z');
}

static bool SelectionContainsCjk(const char* s) {
    const char* p = s;
    while (p && *p) {
        char32_t cp = 0;
        if (!Utf8DecodeOne(p, &cp)) {
            break;
        }
...
static bool SelectionIsSingleWord(const char* s) {
    if (!s || !*s) {
        return false;
    }
    const char* p = s;
    ...
    return letterCount > 0 && letterCount <= 48;
}

static DoubaoPromptKind ClassifyDoubaoSelection(const char* s) {
    char* trimmed = str::Dup(s);
    str::TrimWSInPlace(trimmed, str::TrimOpt::Both);
    defer {
        str::Free(trimmed);
    };
    if (str::IsEmpty(trimmed)) {
        return DoubaoPromptKind::Sentence;
    }
    if (SelectionContainsCjk(trimmed)) {
        return DoubaoPromptKind::Chinese;
    }
    if (SelectionIsSingleWord(trimmed)) {
        return DoubaoPromptKind::Word;
    }
    return DoubaoPromptKind::Sentence;
}

static TempStr BuildDoubaoPromptTemp(const char* selection, DoubaoPromptKind kind) {
    char* trimmed = str::Dup(selection);
    str::TrimWSInPlace(trimmed, str::TrimOpt::Both);
    defer {
        str::Free(trimmed);
    };
...
static LRESULT FrameOnCommand(MainWindow* win, HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (cmdId) {
        ...
        case CmdSearchSelectionWithYoudaoDict:
            LaunchBrowserWithSelection(tab, "https://dict.youdao.com/result?word=${selection}&lang=en");
            break;

        case CmdAnalyzeSelectionWithDoubao:
...
static void DrawCaptionMenuSeparator(MainWindow* win, HDC hdc) {
    ButtonInfo& sysBtn = win->captionBtn[CB_SYSTEM_MENU];
    ButtonInfo& menuBtn = win->captionBtn[CB_MENU];
    ...
    int y2 = sysBtn.rect.y + sysBtn.rect.dy - pad;
...
static void DrawCaptionButton(MainWindow* win, HDC hdc, ButtonInfo* bi) {
    ...
    if (isSysButton) {
        ...
        float cy = (float)rc.y + (float)rc.dy / 2.0f;
        float x1 = cx - 8.0f * iconScale;
        float x2 = cx + 8.0f * iconScale;
        float y0 = cy - 6.0f * iconScale;
        float gap = 6.0f * iconScale;
        gfx.DrawLine(&p, x1, y0, x2, y0);
...
static LRESULT CustomCaptionFrameProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, bool* callDef, MainWindow* win) {
    switch (msg) {
        ...
        case WM_PAINT: {
            ...
            DrawCaptionMenuSeparator(win, memDC);
            buffer.Flush(hdc);
...
static void ReadAloudRestartSpeakingFromCurrentPosition(WindowTab* tab) {
    ...
    TtsProcessEvents();
    int posInChunk = TtsGetSpokenPosUtf8();
    if (posInChunk < 0) {
        posInChunk = 0;
    }
...
static bool ReadAloudHasMoreDocumentPages(WindowTab* tab) {
    if (!tab || tab->readAloudBuiltEndPage <= 0) {
        return false;
    }
    DisplayModel* dm = tab->AsFixed();
    if (!dm) {
    ...
    return tab->readAloudBuiltEndPage < dm->PageCount();
}

static bool ReadAloudEnsureDocumentTextExtended(WindowTab* tab) {
    if (!tab || tab->readAloudBuiltEndPage <= 0) {
        return false;
    }
    DisplayModel* dm = tab->AsFixed();
    ...
    int pageCount = dm->PageCount();
    if (tab->readAloudBuiltEndPage >= pageCount) {
        return false;
    }

    int textLen = str::Leni(tab->readAloudText);
    int remaining = textLen - tab->readAloudChunkEnd;
    if (remaining >= kReadAloudMaxChunkLen * 2) {
        return false;
    }

    int nextStart = tab->readAloudBuiltEndPage + 1;
    int nextEnd = nextStart + kReadAloudBuildPagesPerBatch - 1;
...
static bool ReadAloudHasMoreChunks(WindowTab* tab) {
    if (!tab || str::IsEmpty(tab->readAloudText)) {
        return ReadAloudHasMoreDocumentPages(tab);
    }
    if (tab->readAloudChunkEnd < str::Leni(tab->readAloudText)) {
        return true;
    }
    return ReadAloudHasMoreDocumentPages(tab);
}
...
static bool ReadAloudSpeakChunk(WindowTab* tab, const char* errMsg) {
    ...
    if (str::IsEmpty(tab->readAloudText)) {
        if (!ReadAloudEnsureDocumentTextExtended(tab) || str::IsEmpty(tab->readAloudText)) {
            return false;
        }
    }
    ...
    int textLen = str::Leni(tab->readAloudText);
    if (start >= textLen) {
        if (ReadAloudEnsureDocumentTextExtended(tab)) {
            textLen = str::Leni(tab->readAloudText);
        }
        if (start >= textLen) {
            return false;
        }
...
static TempStr CleanReadAloudTextTemp(const char* text) {
    ...
    while (*s) {
        ...
        if (IsReadAloudLineBreak(c)) {
            ...
            bool joinLine = false;
            if (lineBreaks == 1 && outLen > 0) {
                joinLine = ReadAloudShouldJoinAtWrappedLine(out, (int)outLen, s);
            }

            if (!joinLine && !lastWasSpace && outLen > 0) {
...
static void ReadAloudInTab(WindowTab* tab) {
    ...
    ReadAloudPrepareRestart(tab, tab->win);
...
static bool ReadAloudIsPointOnCanvas(MainWindow* win, Point pt) {
    ...
    return ClientRect(win->hwndCanvas).Contains(pt);
}

static void ReadAloudUpdateMenuCursorPoint(MainWindow* win, bool useContextMenuCursorPoint) {
    if (!win) {
        return;
    }
    ...
    DisplayModel* dm = win->AsFixed();
    if (!dm) {
        win->contextMenuPtValid = false;
        return;
    }

    if (useContextMenuCursorPoint) {
        win->readAloudMenuFromContext = true;
        win->contextMenuPtValid = ReadAloudCanReadFromCursor(dm, win->contextMenuPt);
        return;
    }

    win->readAloudMenuFromContext = false;
    ...
    // Opening the menu bar or toolbar dropdown usually leaves the mouse outside
    // the canvas, so prefer the last left-click on text, then last hover position.
    if (win->readAloudLastClickTextPtValid && ReadAloudCanReadFromCursor(dm, win->readAloudLastClickTextPt)) {
        win->contextMenuPt = win->readAloudLastClickTextPt;
        win->contextMenuPtValid = true;
        return;
    }

    if (win->readAloudLastTextPtValid && ReadAloudCanReadFromCursor(dm, win->readAloudLastTextPt)) {
        win->contextMenuPt = win->readAloudLastTextPt;
        win->contextMenuPtValid = true;
        return;
    }

    Point candidates[2] = {win->dragPrevPos, HwndGetCursorPos(win->hwndCanvas)};
    for (Point pt : candidates) {
        if (!ReadAloudIsPointOnCanvas(win, pt)) {
            continue;
        }
        if (ReadAloudCanReadFromCursor(dm, pt)) {
            win->contextMenuPt = pt;
            win->contextMenuPtValid = true;
            return;
        }
    }
    ...
    win->contextMenuPt =
        ReadAloudIsPointOnCanvas(win, win->dragPrevPos) ? win->dragPrevPos : HwndGetCursorPos(win->hwndCanvas);
    win->contextMenuPtValid = false;
}
...
static bool ReadAloudResolveReadFromCursorPoint(MainWindow* win, Point* ptOut) {
    ...
    if (win->readAloudMenuFromContext && win->contextMenuPtValid &&
        ReadAloudCanReadFromCursor(dm, win->contextMenuPt)) {
        *ptOut = win->contextMenuPt;
        return true;
    }

    if (win->readAloudLastClickTextPtValid && ReadAloudCanReadFromCursor(dm, win->readAloudLastClickTextPt)) {
        *ptOut = win->readAloudLastClickTextPt;
        return true;
    }

    ReadAloudUpdateMenuCursorPoint(win, false);
...
static void BuildReadAloudMenuItems(HMENU menu, MainWindow* win, bool useContextMenuCursorPoint) {
    if (win) {
        ReadAloudUpdateMenuCursorPoint(win, useContextMenuCursorPoint);
    }
...
LRESULT CALLBACK WndProcSumatraFrame(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        ...
        case WM_SETTINGCHANGE:
            if (UseDarkModeLib() && DarkMode::handleSettingChange(lp)) {
        ...
        case WM_TTS_EVENT:
            ...
            if (TtsConsumeChunkFinished() && gReadAloudSourceTab) {
