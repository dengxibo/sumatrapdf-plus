...
const fileSettings: Field[] = [
  ...
  notSaved(mkField("ThumbnailBlankKnown", Bool, false, "runtime-only: cached result of IsThumbnailMostlyBlank")),
  notSaved(mkField("ThumbnailIsBlank", Bool, false, "runtime-only: whether thumbnail is mostly blank")),
  notSaved(
  ...
  setVersion(
    setExpert(
      mkField(
        ...
        Float,
        1.0,
        "legacy single speaking rate; kept for migration, prefer ReadAloudSpeakingRateZh/En",
      ),
    ),
    "3.7",
  ),
...
const gLangs: string[][] = [
  ...
  ["bn", "Bengali (??????"],
  ["co", "Corsican (Corsu)"],
  ["br", "Portuguese - Brazil (Portugu?s)"],
  ["bs", "Bosnian (Bosanski)"],
  ["by", "Belarusian (??????????)"],
  ["ca", "Catalan (Catal?)"],
  ["ca-xv", "Catalan-Valencian (Catal?-Valenci?)"],
  ...
  ["pa", "Punjabi (??????)"],
  ["pl", "Polish (Polski)"],
  ["pt", "Portuguese - Portugal (Portugu?s)"],
  ["ro", "Romanian (Rom?n?)"],
  ["ru", "Russian (???????)"],
  ["sat", "Santali (????????"],
  ["si", "Sinhala (??????"],
