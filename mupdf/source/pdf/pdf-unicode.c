...
static void pdf_remap_cmap_range(fz_context* ctx, pdf_cmap* ucs_from_gid, unsigned int cpt, unsigned int gid,
                                 unsigned int n, pdf_cmap* ucs_from_cpt) {
    unsigned int k;
...
static pdf_cmap* pdf_remap_cmap(fz_context* ctx, pdf_cmap* gid_from_cpt, pdf_cmap* ucs_from_cpt) {
    ...
    fz_try(ctx) {
        if (gid_from_cpt->usecmap) ucs_from_gid->usecmap = pdf_remap_cmap(ctx, gid_from_cpt->usecmap, ucs_from_cpt);
 ...
 */
static int unicode_from_coded_glyph_name(const char* name) {
    ...
    if (*end != 0) return 0;
    if (code > 0 && code <= 0xffff) return (int)code;
    return 0;
}
...
static int to_unicode_is_latin1_identity_high(pdf_cmap* cmap) {
    unsigned int c;
    int n_mapped = 0;
    int n_identity = 0;
...
static int cid_to_ucs_is_latin1_identity_high(const unsigned short* cid_to_ucs, size_t len) {
    unsigned int c;
    int n_mapped = 0;
    int n_identity = 0;

    if (!cid_to_ucs || len < 256) return 0;

    for (c = 0xC0; c <= 0xFF; c++) {
        unsigned short u = cid_to_ucs[c];
        if (u == 0 || u == FZ_REPLACEMENT_CHARACTER) continue;
        n_mapped++;
        if (u == c) n_identity++;
    }
...
static int utf8_contains_cyrillic(const char* s) {
    if (!s) return 0;
    while (*s) {
        int rune;
        ...
        if ((rune >= 0x0400 && rune <= 0x04FF) || (rune >= 0x0500 && rune <= 0x052F)) return 1;
        s += n;
    }
    return 0;
}
...
static const char* font_name_without_subset(const char* name) {
    int i;

    if (!name || strlen(name) <= 7 || name[6] != '+') return name;
    for (i = 0; i < 6; i++) {
        char c = name[i];
        if (!((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'))) return name;
    }
    return name + 7;
}

/* Family token before style suffix (-Bold, -Italic, …). */
static size_t font_family_len(const char* name) {
    const char* p = name;
    while (*p && *p != '-' && *p != ',' && *p != ' ') p++;
    return (size_t)(p - name);
}

/* ASCII-only. fz_strncasecmp is UTF-8 and asserts equal rune widths; a
 * byte-stepping substring search on WinAnsi/PDFDoc font names hits that. */
...
static int ascii_contains_ci(const char* hay, const char* needle) {
    ...
    for (; hay[0]; hay++) {
        for (i = 0; i < n; i++) {
            unsigned char hc = (unsigned char)hay[i];
            unsigned char nc = (unsigned char)needle[i];
            if (hc == 0) return 0;
            if (hc >= 'A' && hc <= 'Z') hc = (unsigned char)(hc + 32);
            if (nc >= 'A' && nc <= 'Z') nc = (unsigned char)(nc + 32);
            if (hc != nc) break;
        }
        if (i == n) return 1;
    }
...
static int font_name_suggests_cyrillic(const char* name) {
    ...
    static const char* families[] = {
        "literaturnaya", "academy",  "schoolbook",  "petersburg", "freeset", "pragmatica", "newton",
        "baltica",       "magazine", "zhurnalnaya", "zhurnal",    "journal", "kudryashev", "kudriashov",
        "octava",        "textbook", "certum",      "pushkin",    "karolla", "hermes",     "futuris",
        "gazeta",        "arialcyr", "timescyr",    "courcyr",    NULL};
    const char* base;
    size_t flen;
    int i;
    ...
    if (ascii_contains_ci(base, "cyrillic") || ascii_contains_ci(base, "cyrl") || ascii_contains_ci(base, "paratype") ||
        ascii_contains_ci(base, "cp1251") || ascii_contains_ci(base, "windows-1251"))
        return 1;
    ...
    for (i = 0; families[i]; i++) {
        ...
        if (flen == n && ascii_ncasecmp(base, families[i], n) == 0) return 1;
    }
    return 0;
}

static int pdf_doc_suggests_cyrillic(fz_context* ctx, pdf_document* doc) {
    static const char* info_keys[] = {FZ_META_INFO_TITLE, FZ_META_INFO_SUBJECT, FZ_META_INFO_AUTHOR,
                                      FZ_META_INFO_KEYWORDS, NULL};
    pdf_obj* root;
    pdf_obj* lang;
    char buf[2048];
    int i;
    ...
    if (pdf_is_string(ctx, lang)) {
        ...
        if (l && l[0] && l[1]) {
            char a = l[0] | 0x20;
            char b = l[1] | 0x20;
            if ((a == 'r' && b == 'u') || (a == 'u' && b == 'k') || (a == 'b' && b == 'g') || (a == 'b' && b == 'e') ||
                (a == 's' && b == 'r') || (a == 'm' && b == 'k'))
                return 1;
        }
    }
    ...
    for (i = 0; info_keys[i]; i++) {
        if (pdf_lookup_metadata(ctx, doc, info_keys[i], buf, sizeof buf) > 0 && utf8_contains_cyrillic(buf)) return 1;
    }
    return 0;
}
...
static void pdf_apply_windows_1251_unicode(fz_context* ctx, pdf_font_desc* font) {
    unsigned int c;
    ...
    for (c = 0; c < 256; c++) font->cid_to_ucs[c] = fz_unicode_from_windows_1251[c];
}
...
static void pdf_fix_cyrillic_cp1251_tounicode(fz_context* ctx, pdf_document* doc, pdf_font_desc* font) {
    int high_is_latin1;
    const char* fname = NULL;
    int cyrillic_hint;

    high_is_latin1 = to_unicode_is_latin1_identity_high(font->to_unicode);
    if (!high_is_latin1) high_is_latin1 = cid_to_ucs_is_latin1_identity_high(font->cid_to_ucs, font->cid_to_ucs_len);
    if (!high_is_latin1) return;

    if (font->font) fname = fz_font_name(ctx, font->font);

    cyrillic_hint = font_name_suggests_cyrillic(fname);
    if (!cyrillic_hint) cyrillic_hint = pdf_doc_suggests_cyrillic(ctx, doc);
    if (!cyrillic_hint) return;

    pdf_apply_windows_1251_unicode(ctx, font);
}

void pdf_load_to_unicode(fz_context* ctx, pdf_document* doc, pdf_font_desc* font, const char** strings,
                         char* collection, pdf_obj* cmapstm) {
    unsigned int cpt;

    if (pdf_is_stream(ctx, cmapstm)) {
        pdf_cmap* ucs_from_cpt = pdf_load_embedded_cmap(ctx, doc, cmapstm);
        fz_try(ctx) font->to_unicode = pdf_remap_cmap(ctx, font->encoding, ucs_from_cpt);
        fz_always(ctx) pdf_drop_cmap(ctx, ucs_from_cpt);
