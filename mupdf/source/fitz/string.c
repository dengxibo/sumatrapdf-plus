...
static const int* fz_ucd_bsearch(int c, const int* t, int n, int ne) {
    const int* p;
    int m;
    while (n > 1) {
        m = n / 2;
...
int fz_strncasecmp(const char* a, const char* b, size_t n) {
    while (n > 0) {
        ...
        /* Different UTF-8 widths cannot be casefold-equal. Call sites often
         * compare ASCII needles (e.g. "Title") against CJK haystacks; do not
         * assert — return unequal. Continuing would also break n accounting
         * below (n -= n_a only). */
        if (n_a != n_b) {
            if (ucs_a == 0 || ucs_b == 0) return ucs_a - ucs_b;
            ucs_a = fz_tolower(ucs_a);
            ucs_b = fz_tolower(ucs_b);
            if (ucs_a != ucs_b) return ucs_a - ucs_b;
            return n_a - n_b;
        }
...
char* fz_strsep(char** stringp, const char* delim) {
    char* ret = *stringp;
    if (!ret) return NULL;
    if ((*stringp = strpbrk(*stringp, delim)) != NULL) *((*stringp)++) = '\0';
    return ret;
}
...
char* fz_realpath(const char* path, char* buf) {
    ...
    int i;
    if (!MultiByteToWideChar(CP_UTF8, 0, path, -1, wpath, PATH_MAX)) return NULL;
    if (!GetFullPathNameW(wpath, PATH_MAX, wbuf, NULL)) return NULL;
    if (!WideCharToMultiByte(CP_UTF8, 0, wbuf, -1, buf, PATH_MAX, NULL, NULL)) return NULL;
    for (i = 0; buf[i]; ++i)
        if (buf[i] == '\\') buf[i] = '/';
    return buf;
}
...
static char* fz_encode_uri_imp(fz_context* ctx, const char* s, const char* unescaped) {
...
void fz_format_output_path(fz_context* ctx, char* path, size_t size, const char* fmt, int page) {
    ...
    for (i = 0; page; page /= 10) num[i++] = '0' + page % 10;
    num[i] = 0;
    ...
    if (p && *p == 'd') {
        ...
        if (psep != NULL && p < psep) p = NULL;
        if (!p) s = p = fmt + strlen(fmt);
    }

    if (z < 1) z = 1;
    while (i < z && i < (int)sizeof num) num[i++] = '0';
    n = s - fmt;
    if (n + i + strlen(p) >= size) fz_throw(ctx, FZ_ERROR_ARGUMENT, "path name buffer overflow");
    memcpy(path, fmt, n);
    while (i > 0) path[n++] = num[--i];
...
int fz_runeidx(const char* s, const char* p) {
    int rune;
    int i = 0;
    while (s < p) {
...
const char* fz_runeptr(const char* s, int i) {
    int rune;
    while (i-- > 0) {
...
size_t fz_atoz(const char* s) {
    ...
    if (s == NULL) return 0;
    i = atoll(s);
    if (i < 0 || (int64_t)(size_t)i != i) return 0;
    return (size_t)i;
}
...
static char* twoway_memmem(const unsigned char* h, const unsigned char* z, const unsigned char* n, size_t l) {
    ...
    for (i = 0; i < l; i++) BITOP(byteset, n[i], |=), shift[n[i]] = i + 1;
...
const char* fz_strstrcase(const char* haystack, const char* needle) {
    ...
    while (1) {
        ...
        if (matchlen == 0) firstlen = nc;
        haystack += nc;
        matchlen += nd;
        if (c == 0) return NULL;
        if (c != d) haystack -= matchlen - firstlen, matchlen = 0;
    }
...
int fz_strverscmp(const char* l0, const char* r0) {
