...
static void layout_line(fz_context* ctx, float indent, float page_w, float line_w, int align, fz_html_flow* start,
    ...
    /* EPUBs often wrap illustrations in their own <p><img/></p> without text-align:center.
     * Center lone images on the line, matching HtmlFormatter::LayoutLeftStartingAt. */
    if (slop > 0 && align != TA_RIGHT && align != TA_CENTER) {
        int image_only = 1;
...
/* When horizontal margins are 'auto' and width is specified, absorb the
 * leftover space in the containing block (CSS2.1 block centering). mupdf
 * previously treated auto margins as zero, breaking EPUB figure wrappers
 * such as Penguin's div.portrait_* { width: 28%; margin: 1.5em auto; }. */
static void resolve_horizontal_auto_margins(fz_html_box* box, float containing_w, float content_w) {
    const fz_css_style* style;
    float* margin;
    float* border;
    float* padding;
    float avail;
    float fixed_ml, fixed_mr;

    if (box->type != BOX_BLOCK && box->type != BOX_TABLE && box->type != BOX_TABLE_CELL) return;
    if (box->style->position != POS_STATIC && box->style->position != POS_RELATIVE) return;

    style = box->style;
    if (style->margin[L].unit != N_AUTO && style->margin[R].unit != N_AUTO) return;
    if (!fz_css_number_defined_not_auto(style->width)) return;

    margin = box->u.block.margin;
    border = box->u.block.border;
    padding = box->u.block.padding;

    fixed_ml = style->margin[L].unit == N_AUTO ? 0 : margin[L];
    fixed_mr = style->margin[R].unit == N_AUTO ? 0 : margin[R];
    avail = containing_w - content_w - border[L] - border[R] - padding[L] - padding[R] - fixed_ml - fixed_mr;
    if (avail <= 0) return;

    if (style->margin[L].unit == N_AUTO && style->margin[R].unit == N_AUTO) {
        margin[L] = avail / 2;
        margin[R] = avail - margin[L];
    } else if (style->margin[L].unit == N_AUTO)
        margin[L] = avail;
    else
        margin[R] = avail;
}
...
static int pre_position(fz_context* ctx, position_data* pd, layout_data* ld, fz_html_box* box, int ignore_width) {
    ...
    if (!WE_ARE_SKIPPING(restart)) resolve_horizontal_auto_margins(box, ld->bounds[R] - ld->bounds[L], pd->width);
    margin = box->u.block.margin;
    ...
    if (!WE_ARE_SKIPPING(restart)) {
        if (box->style->position == POS_STATIC || box->style->position == POS_RELATIVE) {
            if (pd->has_width && box->s.layout.w < pd->width) {
                float max_w = pd->bounds[R] - pd->bounds[L] - margin[L] - margin[R] - border[L] - border[R] -
                              padding[L] - padding[R];
                float w = pd->width;
...
static inline int cell_colspan(fz_html_box* cell) {
...
static int draw_block_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
    ...
    if (box->style->visibility == V_VISIBLE) {
        ...
        if (y1 > y0)
            draw_rect(ctx, dev, ctm, page_top, box->style->background_color, x0, y0 - cell_padding_top, x1,
                      y1 + cell_padding_bot);
