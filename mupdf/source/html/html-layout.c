...
/*
        ...
        The standard CSS box model is used here. Each box has a margin, a border, padding, and content, any of which can
   be zero in size.
        ...
        In our structures, the box->x,y,w,b refer to the bounding box of the content section alone. The
   margins/borders/paddings are applied to the outside of this automatically.

        HOW WE LAYOUT:

        Our implementation is a 'simple' recursive descent of the structure. We know the width of the area we are laying
   out into at each point, and we know the top/left coords of the enclosing block. So we start with a well defined
   left/top point, offset that for margins/borders/padding etc, calculate the width from the enclosing width (via the
   CSS). We set the height of the content to be zero to start with, and after laying out each of (any) child elements,
   we ensure that the base of our box is always large enough to enclose the child's boxes with the appropriate padding.

        VERTICAL MARGIN COLLAPSE:

        The big complexity in this code is the need to do 'vertical margin collapse'. Basically, if we have 2 blocks,
   laid out vertically from one another, the margin between them is collapsed to be the larger of margins of the two
   blocks, rather than the sum of the margins of the two blocks.

        So block A with a margin of 30, and block B with a margin of 20 will be separated by 30, not 50.

        To make matters more complicated, margin collapse can happen 'through' nestings of blocks, and even through
   empty blocks. It cannot happen through blocks with borders, or padding.

        Accordingly, as we recursively pass through our structure laying out, we keep a 'vertical' parameter,
   representing the amount of vertical space to be considered for collapse. This is set to zero if we hit borders or
   padding or non-zero height content boxes, and our margins are adjusted by it.

        While the margins of a block may be adjusted by a parent 'inheriting' those margins, the bbox of the content of
   a block will never be changed.

        NON-RESTARTING OPERATION:

        The standard layout operation as used for HTML documents and ebooks has us layout onto an infinitely long page,
   with the vertical offsets of boxes adjusted so that a given line of text (or image) never spans a page end (defined
   to be a multiple of page_h). This layout can proceed from start to finish with no need to stop. This is achieved by
   calling the code with restart == NULL.

        RESTARTING:

        In order to cope with laying out a text story into multiple (potentially differently sized) areas, we need to be
   able to stop the layout when the available area is full, and restart again later. We do this by having a restart
   record. The tree structure means we can't simply return a pointer to the node to continue processing at - we need to
   recursively redescend the tree until we reach the same position, whereupon we will continue. Also, because of the
   fact that the area being laid out into is potentially a completely different size, we need to recalculate widths etc,
   so at least part of the tree needs to be reprocessed when we restart.

        Accordingly, two ways to do this immediately suggest themselves. The first is to store the path down the tree so
   that we can quickly and efficiently skip whole areas of the tree that do not need reprocessing and redescend just the
   sections we need to. The second is to accept that we will reprocess the entire prefix of the tree each time, and just
   not actually do any work until we reach the exact box where we need to restart.

        We opt for the second one here, on the grounds that 1) it's easier to code, 2) it requires a constant amount of
   storage, and 3) that the trees we will be reprocessing will be relatively small. Should we ever need to revisit this
   decision, we can do so.
*/
...
static int quick_ligature(fz_context* ctx, string_walker* walker, unsigned int i) {
    if (walker->glyph_info[i].codepoint == 'f' && i + 1 < walker->glyph_count &&
        !fz_font_flags(walker->font)->is_mono) {
        if (walker->glyph_info[i + 1].codepoint == 'f') {
            ...
            if (fz_encode_character(ctx, walker->font, 0xFB00)) return quick_ligature_mov(ctx, walker, i, 1, 0xFB00);
        }
        if (walker->glyph_info[i + 1].codepoint == 'i') {
            if (fz_encode_character(ctx, walker->font, 0xFB01)) return quick_ligature_mov(ctx, walker, i, 1, 0xFB01);
        }
        if (walker->glyph_info[i + 1].codepoint == 'l') {
            if (fz_encode_character(ctx, walker->font, 0xFB02)) return quick_ligature_mov(ctx, walker, i, 1, 0xFB02);
        }
    }
    return walker->glyph_info[i].codepoint;
}

static void init_string_walker(fz_context* ctx, string_walker* walker, hb_buffer_t* hb_buf, int rtl, fz_font* font,
                               int script, int language, int small_caps, const char* text, int cluster_as_graphemes) {
    walker->ctx = ctx;
...
static void destroy_hb_shaper_data(fz_context* ctx, void* handle) {
    fz_hb_lock(ctx);
    hb_font_destroy(handle);
    fz_hb_unlock(ctx);
}

static const hb_feature_t small_caps_feature[1] = {{HB_TAG('s', 'm', 'c', 'p'), 1, 0, (unsigned int)-1}};
...
static int walk_string(string_walker* walker) {
    ...
    while (*walker->s) {
        ...
        (void)fz_encode_character_with_fallback(ctx, walker->base_font, c, walker->script, walker->language,
                                                &walker->next_font);
        if (walker->next_font != walker->font) {
            if (walker->font != NULL) break;
            walker->font = walker->next_font;
        }
        walker->end = walker->s;
    }
    ...
    quickshape = 0;
    if (walker->script <= 3 && !walker->rtl && !fz_font_flags(walker->font)->has_opentype) quickshape = 1;
    ...
    fz_try(ctx) {
        ...
        if (fterr) fz_throw(ctx, FZ_ERROR_LIBRARY, "freetype setting character size: %s", ft_error_string(fterr));
...
static void measure_string_w(fz_context* ctx, fz_html_flow* node, hb_buffer_t* hb_buf) {
    ...
    init_string_walker(ctx, &walker, hb_buf, node->bidi_level & 1, node->box->style->font, node->script,
                       node->markup_lang, node->box->style->small_caps, s, 0);
    while (walk_string(&walker)) {
        int x = 0;
        for (i = 0; i < walker.glyph_count; i++) x += walker.glyph_pos[i].x_advance;
...
static void layout_line(fz_context* ctx, float indent, float page_w, float line_w, int align, fz_html_flow* start,
                        fz_html_flow* end, fz_html_box* box, float baseline, float line_h) {
    ...
    for (i = 0, n = 0, node = start; node != end; node = node->next) {
        n++;
        if (node->type == FLOW_SPACE && node->expand && !node->breaks_line) i++;
    }
    ...
    for (i = 0, node = start; node != end; i++, node = node->next) {
        reorder[i] = node;
        if (node->bidi_level < min_level) min_level = node->bidi_level;
        if (node->bidi_level > max_level) max_level = node->bidi_level;
    }
    ...
    if (min_level != max_level || (min_level & 1)) {
        ...
        do {
            ...
            do {
                ...
                while (start_idx < n && reorder[start_idx]->bidi_level < max_level) start_idx++;
                /* If start >= n-1 then no more runs. */
                if (start_idx >= n - 1) break;
                /* Find the end of the match */
                i = start_idx + 1;
                while (i < n && reorder[i]->bidi_level >= max_level) i++;
        ...
        if (node->type == FLOW_IMAGE) {
            switch (node->box->style->vertical_align) {
                ...
                case VA_LENGTH:
                    node->y = y + baseline - node->h -
                              fz_from_css_number(
                                  node->box->style->vertical_align_number, node->box->s.layout.em,
                                  fz_from_css_number_scale(node->box->style->line_height, node->box->s.layout.em), 0);
                    break;
            }
        } else {
            switch (node->box->style->vertical_align) {
    ...
    if (box->s.layout.x + box->s.layout.w < x) box->s.layout.w = x - box->s.layout.x;
}
...
static int flush_line(fz_context* ctx, fz_html_box* box, layout_data* ld, float line_w, int align, float indent,
    ...
    if (page_h > 0) {
        ...
        if (line_h > avail) {
            ...
            layout_extend_bounds_to_page_bottom(ld);
...
static void break_word_for_overflow_wrap(fz_context* ctx, fz_html_flow* node, layout_data* ld, float max_w) {
    ...
    if ((node->bidi_level & 1) == 0) {
        /* Left 2 Right */
        init_string_walker(ctx, &walker, hb_buf, 0 /* L2R */, node->box->style->font, node->script, node->markup_lang,
                           node->box->style->small_caps, text, 1);
        while (walk_string(&walker)) {
            unsigned int i;
#ifdef DEBUG_DESPERATE_SPLITTING
            for (i = 0; i < walker.glyph_count; ++i) {
                uint32_t can_break_here =
                    (hb_glyph_info_get_glyph_flags(&walker.glyph_info[i]) & HB_GLYPH_FLAG_UNSAFE_TO_BREAK) == 0;
                printf("%s(%x, %d)", can_break_here ? "|" : " ", walker.glyph_info[i].codepoint,
                       walker.glyph_info[i].cluster);
            }
            printf("\n");
#endif
            for (i = 0; i < walker.glyph_count; ++i) {
        ...
        /* Right 2 Left */
        init_string_walker(ctx, &walker, hb_buf, 1 /* R2L */, node->box->style->font, node->script, node->markup_lang,
                           node->box->style->small_caps, text, 1);
        while (walk_string(&walker)) {
            unsigned int i;
#ifdef DEBUG_DESPERATE_SPLITTING
            ...
            if (w > max_w) break;
            if (walker.start != text) at = walker.start - text;
...
static void layout_flow(fz_context* ctx, layout_data* ld, fz_html_box* box, fz_html_box* top) {
    ...
    for (; node; node = node->next) {
        ...
        if (node->type == FLOW_IMAGE) {
            ...
            float aspect = 1;
            float page_h;
            float intr_w, intr_h, est_w, est_h;

            /* If this figure won't fit in the page remainder, start on a fresh page
             * (cnepub withnote figures lack page-break in publisher CSS). */
            intr_w = node->content.image->w * 72.0f / 96.0f;
            intr_h = node->content.image->h * 72.0f / 96.0f;
            est_w = intr_w;
            est_h = intr_h;
            ...
            layout_flow_image_page_break(ctx, ld, box, node->box, node->box->style->page_break_before);

            page_h = ld->bounds[B] - ld->bounds[T];
            ...
            if (node->box->style->width.unit != N_AUTO)
                node->w = fz_from_css_number(node->box->style->width, top->s.layout.em, ld->bounds[R] - ld->bounds[L],
                                             node->w);
            ...
            if (max_w > 0 && node->w > max_w) xs = max_w / node->w;
            if (max_h > 0 && node->h > max_h) ys = max_h / node->h;
            s = fz_min(xs, ys);
            node->w = node->w * s;
            node->h = node->h * s;

            /* Never taller than one reflow page (avoids clipped split across fz pages). */
            {
                float slice_h = ld->page[B] - ld->page[T];
                float avail_h = layout_page_remainder(ld, ld->bounds[T]);
                if (avail_h > 0 && avail_h < slice_h) slice_h = avail_h;
                if (slice_h > 0 && node->h > slice_h) {
                    float t = slice_h / node->h;
                    node->w *= t;
                    node->h = slice_h;
                }
                /* auto height + publisher % width (cnepub withnote): leave room for caption. */
                if (slice_h > 0 && node->box->style->height.unit == N_AUTO &&
                    node->box->style->width.unit == N_PERCENT) {
    ...
    while (node) {
        ...
        if (node->type == FLOW_BREAK || (line_w > bounds_w && (candidate || desperate))) {
            ...
            if (candidate) {
                if (candidate->type == FLOW_BREAK) line_align = (align == TA_JUSTIFY) ? justify_align : align;
            ...
            if (box->markup_dir == FZ_BIDI_RTL) indent = 0;

            if (flush_line(ctx, box, ld, break_w, line_align, indent, line, break_at, restart)) return;
...
static int layout_block_page_break(fz_context* ctx, layout_data* ld, float* yp, int page_break) {
    ...
    if (WE_ARE_SKIPPING(restart)) return 0;

    /* If the page height is screwy (or the rectangle height), then bale. */
    if (page_h <= 0) return 0;

    /* If we aren't doing some kind of page break, bale. */
    if (page_break != PB_ALWAYS && page_break != PB_LEFT && page_break != PB_RIGHT) return 0;
    ...
    if (avail == page_h) {
        if (page_break == PB_ALWAYS) return 0;
        if (page_break == PB_LEFT && side == 1) return 0;
        if (page_break == PB_RIGHT && side == 0) return 0;
    }

    /* Move us to the next page. */
    if (avail > 0) *yp += avail, side ^= 1;
    ...
    if (page_break == PB_LEFT) {
        ...
        if (SHOULD_STOP_FOR_RESTART(restart))
            restart->end_flags =
                FZ_HTML_RESTARTER_START_END_FLAGS_SPECIFIC_SIDE | FZ_HTML_RESTARTER_START_END_FLAGS_LEFT_SIDE;
        if (side == 0) /* right pages, side == 0 */
            *yp += page_h;
    }
    if (page_break == PB_RIGHT) {
        /* If we're restarting, make sure we only restart on a right page. */
        if (SHOULD_STOP_FOR_RESTART(restart)) restart->end_flags = FZ_HTML_RESTARTER_START_END_FLAGS_SPECIFIC_SIDE;
...
static void layout_extend_bounds_to_page_bottom(layout_data* ld) {
    float page_h = ld->page[B] - ld->page[T];
    ld->bounds[B] = ld->bounds[T] + page_h;
}

static int layout_flow_image_page_break(fz_context* ctx, layout_data* ld, fz_html_box* flow_box, fz_html_box* img_box,
                                        int page_break) {
    if (layout_block_page_break(ctx, ld, &ld->bounds[T], page_break)) {
        layout_extend_bounds_to_page_bottom(ld);
        flow_box->s.layout.y = ld->bounds[T];
        flow_box->s.layout.b = ld->bounds[T];
        return 1;
    }
    return 0;
}
 ...
 */
static void shift_box_contents(fz_html_box* box, float x, float y) {
    fz_html_box* child;
    ...
    if (box->type == BOX_FLOW) {
        fz_html_flow* flow;
        for (flow = box->u.flow.head; flow != NULL; flow = flow->next) flow->x += x, flow->y += y;
    }
    for (child = box->down; child; child = child->next) {
        if (child->style->position == POS_FIXED || child->style->position == POS_ABSOLUTE) return;
 ...
 */
static int pre_position(fz_context* ctx, position_data* pd, layout_data* ld, fz_html_box* box, int ignore_width) {
    float em = box->s.layout.em;
    ...
    if (WE_ARE_SKIPPING(restart)) {
        ...
        if (eop && SHOULD_STOP_FOR_RESTART(restart)) return 1;
    }
    ...
    /* Adjust horizontally for Width and spacings.
       Percentage width is of the containing block (CSS 2.1 10.1), not of
       the area after auto margins. Otherwise width:64% + margin:auto
       shrinks to ~41% and sits left of center. */
    {
        float containing_w = ld->bounds[R] - ld->bounds[L];
        auto_w = containing_w - (margin[L] + margin[R] + border[L] + border[R] + padding[L] + padding[R]);
    ...
    if (!WE_ARE_SKIPPING(restart)) {
        if (box->style->position == POS_STATIC || box->style->position == POS_RELATIVE) {
            if (pd->has_width && box->s.layout.w < pd->width) box->s.layout.w = pd->width;
    ...
    if (box->style->position == POS_STATIC) return eop;
 ...
 *	Returns eop
 */
static int post_position(fz_context* ctx, position_data* pd, layout_data* ld, fz_html_box* box, int widths_set) {
    int eop = 0; /* dummy */
    fz_html_restarter* restart = ld->restart;
    float em = box->s.layout.em;
    const float* margin = box->u.block.margin;
    ...
    if (ld->used[B] > box->s.layout.b) box->s.layout.b = ld->used[B];

    /* Unless we're still skipping, add the padding/border/margin on the bottom. */
    if (!WE_ARE_SKIPPING(restart)) {
        if (box->style->position == POS_STATIC || box->style->position == POS_RELATIVE) {
            if (pd->has_width && box->s.layout.w < pd->width && !widths_set) box->s.layout.w = pd->width;
            if (pd->has_height && box->s.layout.b - box->s.layout.y < pd->height)
                box->s.layout.b = pd->height + box->s.layout.y;
        }

        ld->used[B] = advance_for_spacing(
            ctx, ld, ld->used[B], box->u.block.padding[B] + box->u.block.border[B] + box->u.block.margin[B], &eop);
    }
    ...
    ld->used[L] = box->s.layout.x;
    if (ld->used[B] < box->s.layout.b) ld->used[B] = box->s.layout.b;
...
static void resize_grid(fz_context* ctx, table_grid* grid, int w, int h) {
    ...
    if (grid->max_h != h) memset(&grid->cells[w * grid->max_h], 0, sizeof(grid->cells[0]) * w * (h - grid->max_h));
    grid->max_w = w;
    grid->max_h = h;
}
...
static column_width* table_grid_complete(fz_context* ctx, table_grid* grid, float top) {
    int w = grid->w;
    int h = grid->h;
    int x, y, x2;
    ...
    for (x = 0; x < w; x++) {
        ...
        for (y = 0; y < h; y++) {
            ...
            if (cell->spanned || cell->colspan > 1) continue;
            if (cell->fixed) {
                fixed = 1;
                minw = cell->minw;
                maxw = cell->maxw;
                ...
                if (!found || cell->minw > minw) minw = cell->minw;
                if (cell->maxw > maxw) maxw = cell->maxw;
                found = 1;
            }
        }
        if (found) {
        ...
        for (x = 0; x < w; x++) {
            ...
            if (minw < cell->minw) {
                ...
                if (unknown > 0) {
                    ...
                    boost /= nonfixed;
                    for (x2 = x; x2 < x + cell->colspan; x2++) {
                        if (colw[x2].fixed) continue;
                        colw[x2].fixed = cell->fixed;
                        colw[x2].min += boost;
                        if (colw[x2].max < colw[x2].min) colw[x2].max = colw[x2].min;
                    }
                } else {
     ...
     */
    for (x = 0; x < w; x++) {
        if (colw[x].fixed == -1) colw[x].fixed = 0;
    }
...
static float table_cell_padding(fz_context* ctx, fz_html_box* box) {
    return (box->u.block.padding[L] + box->u.block.border[L] + box->u.block.padding[R] + box->u.block.border[R]);
}
...
static float table_cell_padding_collapsed(fz_context* ctx, fz_html_box* box, border_collapse_info* bci, int leftmost,
                                          int rightmost) {
    ...
    return (box->u.block.padding[L] + left + box->u.block.padding[R] + right);
}

static float block_padding(fz_context* ctx, fz_html_box* box) {
    return (box->u.block.padding[L] + box->u.block.border[L] + box->u.block.margin[L] + box->u.block.padding[R] +
            box->u.block.border[R] + box->u.block.margin[R]);
}
...
static void squish_block(fz_context* ctx, fz_html_box* box) {
    ...
    if (box->type == BOX_FLOW) return;
    for (child = box->down; child; child = child->next) squish_block(ctx, child);
}

static void layout_table_row(fz_context* ctx, layout_data* ld, table_grid* grid, int celly, fz_html_box* row, int ncol,
                             column_width* colw, float spacing, border_collapse_info* border_collapse) {
    ...
    if (fz_css_number_defined_not_auto(row->style->height)) {
        float row_avail_h = ld->bounds[B] - ld->bounds[T];
        row_height = fz_from_css_number(row->style->height, em, row_avail_h, 0);
        fixed_height = 1;
    }
    ...
    for (cell = row->down; cell; cell = cell->next) {
        if (max_cell_border_above < cell->u.block.border[T]) max_cell_border_above = cell->u.block.border[T];
        if (max_cell_border_below < cell->u.block.border[B]) max_cell_border_below = cell->u.block.border[B];
    }
    ...
    for (cell = row->down; cell; cell = cell->next) {
        ...
        cellw = 0;
        for (i = 0; i < colspan; i++) cellw += colw[col + i].actual + (i != 0 ? spacing : 0);
        ...
        if (fz_css_number_defined_not_auto(cell->style->height)) {
            float cell_avail_h = ld->bounds[B] - ld->bounds[T];
            float cellheight = fz_from_css_number(cell->style->height, em, cell_avail_h, 0);
            if (cell->s.layout.b < cell->s.layout.y + cellheight) cell->s.layout.b = cell->s.layout.y + cellheight;
...
static inline int cell_rowspan(fz_html_box* cell) {
    ...
    for (row0 = 0, rowbox = box->down; row0 <= row && rowbox; row0++, rowbox = rowbox->next) {
        for (cell = rowbox->down; cell; cell = cell->next) {
            ...
            if (cell->style->vertical_align == VA_MIDDLE || cell->style->vertical_align == VA_BOTTOM) {
                ...
                if (cell->style->vertical_align == VA_MIDDLE) offset /= 2;
...
static void fixup_cell_bottoms(fz_context* ctx, table_grid* grid, fz_html_box* box) {
    fz_html_box *rowbox, *cell;
    int row;
    ...
    for (row = 0, rowbox = box; row <= grid->h && rowbox; row++, rowbox = rowbox->next) {
        for (cell = rowbox->down; cell; cell = cell->next) {
            ...
            y = grid->row_b[row + rowspan - 1] -
                (cell->u.block.padding[B] + cell->u.block.border[B] + cell->u.block.margin[B]);
            if (cell->style->vertical_align == VA_MIDDLE || cell->style->vertical_align == VA_BOTTOM) {
                float offset = (y - cell->s.layout.b);
                if (cell->style->vertical_align == VA_MIDDLE) offset /= 2;
                shift_box_contents(cell, 0, offset);
...
static void collapse_table_borders(fz_context* ctx, table_grid* grid, border_collapse_info* bci) {
    ...
    for (y = 0; y < h; y++) {
        ...
        if (cell0->spanned == 1 || cell0->box == NULL) continue;
        if (bci->max_left_border < cell0->box->u.block.border[L]) bci->max_left_border = cell0->box->u.block.border[L];
    }
...
static int layout_table(fz_context* ctx, layout_data* ld, fz_html_box* box) {
    ...
    if (border_collapse) spacing = 0;

    if (restart) {
        /* We have reached the restart point */
        if (restart->start == box) restart->start = NULL;
    }

    /* We honour positions on tables. We don't honour positions on elements of tables (like rows/cells etc). */
    eop = layout_block_page_break(ctx, ld, &ld->bounds[T], box->style->page_break_before);
    if (eop) {
        layout_extend_bounds_to_page_bottom(ld);
        ld->used[T] = ld->bounds[T];
        ld->used[B] = ld->bounds[T];
    ...
    fz_try(ctx) {
        ...
        if (border_collapse) collapse_table_borders(ctx, table, &bci);
        ...
        for (y = 0; y < table->h; y++) {
            for (x = 0; x < table->w; x++) {
                ...
                if (fz_css_number_defined_not_auto(cell->style->width)) {
                    float em = box->s.layout.em;
                    float table_avail_w = ld->bounds[R] - ld->bounds[L];
                    float w = fz_from_css_number(cell->style->width, em, table_avail_w, 0) + cell_pad;
                    tc->fixed = 1;
                    tc->minw = w;
                    tc->maxw = w;
                } else {
                    ...
                    for (child = cell->down; child; child = child->next) {
                        ...
                        if (min > cellminw) cellminw = min;
                        if (max > cellmaxw) cellmaxw = max;
                    }
                    tc->fixed = 0;
                    tc->minw = cellminw;
                    tc->maxw = cellmaxw;
        ...
        avail_w = fz_from_css_number(box->style->width, box->s.layout.em, ld->bounds[R] - ld->bounds[L],
                                     ld->bounds[R] - ld->bounds[L]);
        ...
        if (min_tabw >= avail_w) {
            ...
            for (col = 0; col < ncol; ++col) colw[col].actual = colw[col].min;
            ...
            for (col = 0; col < ncol; ++col) colw[col].actual = colw[col].max;
        } else {
            ...
            if (D == 0) {
                for (col = 0; col < ncol; ++col) colw[col].actual = colw[col].min + W / ncol;
            } else {
                for (col = 0; col < ncol; ++col)
        ...
        if (!border_collapse) fixup_cell_bottoms(ctx, table, box->down);
    ...
    if (layout_block_page_break(ctx, ld, &ld->used[B], box->style->page_break_after)) {
        if (SHOULD_STOP_FOR_RESTART(restart)) return 1;
    }
...
static int layout_block(fz_context* ctx, layout_data* ld, fz_html_box* box) {
    ...
    if (restart) {
        ...
        if (restart->potential == NULL) restart->potential = box;
    }
    ...
    for (child = box->down; child; child = child->next) {
        ...
        if (child->type == BOX_BLOCK)
            ...
            if (!WE_ARE_SKIPPING(restart)) {
                if (child->s.layout.b > child->s.layout.y) box->s.layout.b = ld->used[B];
            }
        }

        if (!WE_ARE_SKIPPING(restart)) {
            if (ld->used[B] > box->s.layout.b) box->s.layout.b = ld->used[B];
            if (ld->used[R] > box->s.layout.x + box->s.layout.w) box->s.layout.w = ld->used[R] - box->s.layout.x;
            if (ld->used[B] > box->s.layout.b) box->s.layout.b = ld->used[B];
        }

        /* Stop if we've reached the endpoint. */
        if (STOPPING_FOR_RESTART(restart)) break;
        if (restart && box->s.layout.b != box->s.layout.y) restart->potential = NULL;
    ...
    if (STOPPING_FOR_RESTART(restart)) return 0;
    ...
    while (box) {
        ...
        if (box->type != BOX_INLINE && box->type != BOX_FLOW) {
            ...
            if (box->type == BOX_BLOCK || box->type == BOX_TABLE) {
                ...
                /* CSS 2.1 10.3.3: unused horizontal space goes to auto margins.
                   mupdf maps margin:auto to 0, so portrait EPUB figures (width:61%
                   + margin:auto, or inline-block + text-align:center) stay left. */
                if (style->width.unit != N_AUTO && style->width.unit != N_UNDEFINED) {
                    int ml_auto = (style->margin[L].unit == N_AUTO);
                    int mr_auto = (style->margin[R].unit == N_AUTO);
                    if (ml_auto || mr_auto) {
                        float leftover = top_w - box->s.layout.w - border[L] - border[R] - padding[L] - padding[R];
                        if (!ml_auto) leftover -= margin[L];
                        if (!mr_auto) leftover -= margin[R];
        ...
        if (box->down) layout_update_styles(ctx, box->down, box);
...
static void layout_update_widths(fz_context* ctx, fz_html_box* box, fz_html_box* top, hb_buffer_t* hb_buf) {
    while (box) {
        ...
        if (box->down) layout_update_widths(ctx, box->down, box, hb_buf);
...
static int is_empty_block_box(fz_html_box* box) {
    fz_html_box* child;
    if (box->type == BOX_BLOCK) {
        if (box->u.block.padding[T] != 0 || box->u.block.padding[B] != 0) return 0;
        if (box->u.block.border[T] != 0 || box->u.block.border[B] != 0) return 0;
        for (child = box->down; child; child = child->next) {
            if (child->type != BOX_BLOCK) return 0;
...
static void layout_collapse_margin_with_children(fz_context* ctx, fz_html_box* here) {
    ...
    if (here->style->position == POS_FIXED || here->style->position == POS_ABSOLUTE) return;
...
static void layout_collapse_margin_with_siblings(fz_context* ctx, fz_html_box* here) {
    ...
    while (here) {
        if (here->down) layout_collapse_margin_with_siblings(ctx, here->down);
...
static void layout_collapse_margin_with_self(fz_context* ctx, fz_html_box* here) {
    while (here) {
        if (here->down) layout_collapse_margin_with_self(ctx, here->down);
...
void fz_restartable_layout_html(fz_context* ctx, fz_html_tree* tree, float start_x, float start_y, float page_w,
                                float page_h, float em, fz_html_restarter* restart) {
    int unlocked = 0;
    layout_data ld = {0};
...
void fz_layout_html(fz_context* ctx, fz_html* html, float w, float h, float em) {
     ...
     * nothing to do. */
    if (html->layout_w == w && html->layout_h == h && html->layout_em == em) return;
    ...
    if (html->page_w <= 72) html->page_w = 72; /* enforce a minimum page size! */
    if (h > 0) {
        html->page_h = h - html->page_margin[T] - html->page_margin[B];
        if (html->page_h <= 72) html->page_h = 72; /* enforce a minimum page size! */
    } else {
...
#ifndef NDEBUG
    if (fz_atoi(getenv("FZ_DEBUG_HTML"))) fz_debug_html(ctx, html->tree.root);
#endif
}
...
static inline unpacked_color unpack_color(const fz_css_color src) {
    unpacked_color dst;
...
static inline int color_eq(fz_css_color a, fz_css_color b) {
    return a.r == b.r && a.g == b.g && a.b == b.b && a.a == b.a;
}

static fz_css_color find_inline_background_color(fz_context* ctx, fz_html_box* box) {
    while (box && box->type == BOX_INLINE) {
        if (box->style->background_color.a > 0) return box->style->background_color;
        box = box->up;
    }
    return (fz_css_color){0, 0, 0, 0};
}

static int draw_flow_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
                         fz_matrix ctm, hb_buffer_t* hb_buf, fz_html_restarter* restart) {
    fz_html_flow* node;
    fz_text* text = NULL;
    fz_path* line = NULL;
    fz_matrix trm;
    ...
    if (restart && restart->start != NULL && restart->start != box) return 0;
        ...
        for (node = box->u.flow.head; node; node = node->next) {
            ...
            if (restart) {
                if (restart->start_flow != NULL) {
                    if (restart->start_flow != node) continue;
                    restart->start = NULL;
                    restart->start_flow = NULL;
                }
            ...
            if (node->type == FLOW_WORD || node->type == FLOW_SPACE || node->type == FLOW_SHYPHEN) {
                ...
                if (node->type == FLOW_SPACE && node->breaks_line) continue;
                if (node->type == FLOW_SHYPHEN && !node->breaks_line) continue;
                if (style->visibility != V_VISIBLE) continue;

                em = node->box->s.layout.em;

                bg = find_inline_background_color(ctx, node->box);
                if (bg.a > 0)
                    draw_rect(ctx, dev, ctm, page_top, bg, node->x, node->y - 0.8f * node->h, node->x + node->w,
                              node->y + 0.2f * node->h);
                ...
                if (stroking) {
                    if (ss == NULL) ss = fz_new_stroke_state(ctx);
                }
                if (line) {
                    if (line_ss == NULL) line_ss = fz_new_stroke_state(ctx);
                }
                ...
                if (/* If we've changed whether we're filling... */
                    ...
                    (prev_stroking &&
                     (!color_eq(style->text_stroke_color, prev_stroke_color) || line_width != prev_line_width))) {
                    if (text) {
                        if (prev_filling) {
                            unpacked_color color = unpack_color(prev_fill_color);
                            fz_fill_text(ctx, dev, text, ctm, fz_device_rgb(ctx), color.rgb, color.a,
                                         fz_default_color_params);
                        }
                        if (prev_stroking) {
                            unpacked_color color = unpack_color(prev_stroke_color);
                            ss->linewidth = prev_line_width;
                            fz_stroke_text(ctx, dev, text, ss, ctm, fz_device_rgb(ctx), color.rgb, color.a,
                                           fz_default_color_params);
                        }
                ...
                if (!color_eq(style->color, prev_color)) {
                    if (line) {
                        ...
                        fz_stroke_path(ctx, dev, line, line_ss, ctm, fz_device_rgb(ctx), color.rgb, color.a,
                                       fz_default_color_params);
                ...
                s = get_node_text(ctx, node);
                init_string_walker(ctx, &walker, hb_buf, node->bidi_level & 1, style->font, node->script,
                                   node->markup_lang, style->small_caps, s, 0);
                while (walk_string(&walker)) {
                    ...
                    if (node->bidi_level & 1) x -= x_advance * node_scale;
                    ...
                    while (walker.start + k < walker.end) {
                        ...
                        if (node->type == FLOW_SHYPHEN) c = 0xAD;
                        ...
                        for (i = 0; i < walker.glyph_count; ++i) {
                            if (walker.glyph_info[i].cluster == k) {
                                ...
                                fz_show_glyph(ctx, text, walker.font, trm, walker.glyph_info[i].codepoint, c, 0,
                                              node->bidi_level, box->markup_dir, node->markup_lang);
                        ...
                        if (c != -1) {
                            fz_show_glyph(ctx, text, walker.font, trm, -1, c, 0, node->bidi_level, box->markup_dir,
                                          node->markup_lang);
                        }
                    ...
                    if ((node->bidi_level & 1) == 0) x += x_advance * node_scale;
...
static void draw_quad(fz_context* ctx, fz_device* dev, fz_matrix ctm, fz_css_color color, float x0, float y0, float x1,
...
static void fudge_len(float* wp, float len, int count, float extra) {
    ...
    if (len > w * extra) n = ((len - w * extra) + (count * w) / 2) / (count * w);
    if (n < 1) n = 1;
...
static void format_roman_number(fz_context* ctx, char* buf, int size, int n, const char* sym[3][10],
                                const char* sym_m) {
    int I = n % 10;
    int X = (n / 10) % 10;
    int C = (n / 100) % 10;
    ...
    fz_strlcpy(buf, "", size);
    while (M--) fz_strlcat(buf, sym_m, size);
...
static void format_alpha_number(fz_context* ctx, char* buf, int size, int n, int alpha, int omega) {
    ...
    while (i > 0) buf += fz_runetochar(buf, tmp[--i]);
    *buf++ = '.';
    *buf++ = ' ';
    *buf = 0;
}
...
static void format_list_number(fz_context* ctx, int type, int x, char* buf, int size) {
    switch (type) {
        case LST_NONE:
            ...
            break;
        case LST_DISC:
            fz_snprintf(buf, size, "%C  ", 0x2022);
            break; /* U+2022 BULLET */
        case LST_CIRCLE:
            fz_snprintf(buf, size, "%C  ", 0x25CB);
            break; /* U+25CB WHITE CIRCLE */
        case LST_SQUARE:
            fz_snprintf(buf, size, "%C  ", 0x25A0);
            break; /* U+25A0 BLACK SQUARE */
        default:
        case LST_DECIMAL:
            fz_snprintf(buf, size, "%d. ", x);
            break;
        case LST_DECIMAL_ZERO:
            fz_snprintf(buf, size, "%02d. ", x);
            break;
        case LST_LC_ROMAN:
            format_roman_number(ctx, buf, size, x, roman_lc, "m");
            break;
        case LST_UC_ROMAN:
            format_roman_number(ctx, buf, size, x, roman_uc, "M");
            break;
        case LST_LC_ALPHA:
            format_alpha_number(ctx, buf, size, x, 'a', 'z');
            break;
        case LST_UC_ALPHA:
            format_alpha_number(ctx, buf, size, x, 'A', 'Z');
            break;
        ...
        case LST_UC_LATIN:
            ...
            break;
        case LST_LC_GREEK:
            format_alpha_number(ctx, buf, size, x, 0x03B1, 0x03C9);
            break;
        case LST_UC_GREEK:
            format_alpha_number(ctx, buf, size, x, 0x0391, 0x03A9);
            break;
    }
}
...
static fz_html_flow* find_list_mark_anchor(fz_context* ctx, fz_html_box* box) {
    ...
    while (box) {
        if (box->type == BOX_FLOW) return box->u.flow.head;
        box = box->down;
    }
    return NULL;
}

static void draw_list_mark(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
                           fz_matrix ctm, int n) {
    fz_font* font;
    fz_text* text;
    fz_matrix trm;
    ...
    if (y > page_bot || y < page_top) return;
...
static int draw_block_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
                          fz_matrix ctm, hb_buffer_t* hb_buf, fz_html_restarter* restart);
static int draw_table_row(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
                          fz_matrix ctm, hb_buffer_t* hb_buf, fz_html_restarter* restart);

static int draw_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev, fz_matrix ctm,
                    hb_buffer_t* hb_buf, fz_html_restarter* restart) {
    int ret = 0;
    int str = fz_html_tag_to_structure(box->tag);

    if (str != FZ_STRUCTURE_INVALID) fz_begin_structure(ctx, dev, str, box->tag, 0);
    ...
    switch (box->type) {
        ...
        case BOX_BLOCK:
            ...
            break;
        case BOX_FLOW:
            if (draw_flow_box(ctx, box, page_top, page_bot, dev, ctm, hb_buf, restart)) ret = 1;
            break;
    }

    if (str != FZ_STRUCTURE_INVALID) fz_end_structure(ctx, dev);

    return ret;
}

static fz_css_color brighten(fz_css_color c) {
    int delta;
    float S;
    float C;
    float H;
...
static fz_css_color darken(fz_css_color c) {
    int delta;
    float S;
    float C;
    float H;
...
static int collapse_border_style(int style, int collapsed) {
    if (collapsed) {
        if (style == BS_INSET) style = BS_RIDGE;
        if (style == BS_OUTSET) style = BS_GROOVE;
    }
    return style;
}

static void draw_border(fz_context* ctx, fz_device* dev, fz_matrix ctm, fz_html_box* box, int edge, float x0, float y0,
                        float x1, float y1) {
    float* border = box->u.block.border;
    const fz_css_style* style = box->style;
    ...
    switch (edge) {
        case T:
            switch (collapse_border_style(style->border_style_0, collapsed)) {
                ...
                case BS_OUTSET:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[T]), x0 - border[L], y0 - border[T], x0, y0,
                              x1, y0, x1 + border[R], y0 - border[T]);
                    break;
                case BS_INSET:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[T]), x0 - border[L], y0 - border[T], x0, y0, x1,
                              y0, x1 + border[R], y0 - border[T]);
                    break;
                case BS_RIDGE:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[T]), x0 - border[L], y0 - border[T],
                              x0 - border[L] / 2, y0 - border[T] / 2, x1 + border[R] / 2, y0 - border[T] / 2,
                              x1 + border[R], y0 - border[T]);
                    draw_quad(ctx, dev, ctm, darken(style->border_color[T]), x0 - border[L] / 2, y0 - border[T] / 2, x0,
                              y0, x1, y0, x1 + border[R] / 2, y0 - border[T] / 2);
                    break;
                case BS_GROOVE:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[T]), x0 - border[L], y0 - border[T],
                              x0 - border[L] / 2, y0 - border[T] / 2, x1 + border[R] / 2, y0 - border[T] / 2,
                              x1 + border[R], y0 - border[T]);
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[T]), x0 - border[L] / 2, y0 - border[T] / 2,
                              x0, y0, x1, y0, x1 + border[R] / 2, y0 - border[T] / 2);
                    break;
                case BS_DOUBLE:
                    draw_quad(ctx, dev, ctm, style->border_color[T], x0 - border[L], y0 - border[T],
                              x0 - 2 * border[L] / 3, y0 - 2 * border[T] / 3, x1 + 2 * border[R] / 3,
                              y0 - 2 * border[T] / 3, x1 + border[R], y0 - border[T]);
                    draw_quad(ctx, dev, ctm, style->border_color[T], x0 - border[L] / 3, y0 - border[T] / 3, x0, y0, x1,
                              y0, x1 + border[R] / 3, y0 - border[T] / 3);
                    break;
                case BS_DOTTED: {
                    float left = border[L] - border[T] / 2;
                    float right = border[R] - border[T] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[T], 0, border[T], x0 - left, y0 - border[T] / 2,
                                     x1 + right, y0 - border[T] / 2);
                    break;
                }
                case BS_DASHED: {
                    float left = border[L] - border[T] / 2;
                    float right = border[R] - border[T] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[T], 1, border[T], x0 - left, y0 - border[T] / 2,
                                     x1 + right, y0 - border[T] / 2);
                    break;
                }
                case BS_SOLID:
        ...
        case R:
            switch (collapse_border_style(style->border_style_1, collapsed)) {
                case BS_NONE:
                    return; /* Should never happen */
                case BS_OUTSET:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[R]), x1 + border[R], y0 - border[T], x1, y0, x1,
                              y1, x1 + border[R], y1 + border[B]);
                    break;
                case BS_INSET:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[R]), x1 + border[R], y0 - border[T], x1, y0,
                              x1, y1, x1 + border[R], y1 + border[B]);
                    break;
                case BS_RIDGE:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[R]), x1 + border[R], y0 - border[T],
                              x1 + border[R] / 2, y0 - border[T] / 2, x1 + border[R] / 2, y1 + border[B] / 2,
                              x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[R]), x1 + border[R] / 2, y0 - border[T] / 2,
                              x1, y0, x1, y1, x1 + border[R] / 2, y1 + border[B] / 2);
                    break;
                case BS_GROOVE:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[R]), x1 + border[R], y0 - border[T],
                              x1 + border[R] / 2, y0 - border[T] / 2, x1 + border[R] / 2, y1 + border[B] / 2,
                              x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, darken(style->border_color[R]), x1 + border[R] / 2, y0 - border[T] / 2, x1,
                              y0, x1, y1, x1 + border[R] / 2, y1 + border[B] / 2);
                    break;
                case BS_DOUBLE:
                    draw_quad(ctx, dev, ctm, style->border_color[R], x1 + border[R], y0 - border[T],
                              x1 + 2 * border[R] / 3, y0 - 2 * border[T] / 3, x1 + 2 * border[R] / 3,
                              y1 + 2 * border[B] / 3, x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, style->border_color[R], x1 + border[R] / 3, y0 - border[T] / 3, x1, y0, x1,
                              y1, x1 + border[R] / 3, y1 + border[B] / 3);
                    break;
                case BS_DOTTED: {
                    float top = border[T] - border[R] / 2;
                    float bottom = border[B] - border[R] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[R], 0, border[R], x1 + border[R] / 2, y0 - top,
                                     x1 + border[R] / 2, y1 + bottom);
                    break;
                }
                case BS_DASHED: {
                    float top = border[T] - border[R] / 2;
                    float bottom = border[B] - border[R] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[R], 1, border[R], x1 + border[R] / 2, y0 - top,
                                     x1 + border[R] / 2, y1 + bottom);
                    break;
                }
                case BS_SOLID:
        ...
        case B:
            switch (collapse_border_style(style->border_style_2, collapsed)) {
                case BS_NONE:
                    return; /* Should never happen */
                case BS_OUTSET:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[B]), x0 - border[L], y1 + border[B], x0, y1, x1,
                              y1, x1 + border[R], y1 + border[B]);
                    break;
                case BS_INSET:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[B]), x0 - border[L], y1 + border[B], x0, y1,
                              x1, y1, x1 + border[R], y1 + border[B]);
                    break;
                case BS_RIDGE:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[B]), x0 - border[L], y1 + border[B],
                              x0 - border[L] / 2, y1 + border[B] / 2, x1 + border[R] / 2, y1 + border[B] / 2,
                              x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[B]), x0 - border[L] / 2, y1 + border[B] / 2,
                              x0, y1, x1, y1, x1 + border[R] / 2, y1 + border[B] / 2);
                    break;
                case BS_GROOVE:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[B]), x0 - border[L], y1 + border[B],
                              x0 - border[L] / 2, y1 + border[B] / 2, x1 + border[R] / 2, y1 + border[B] / 2,
                              x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, darken(style->border_color[B]), x0 - border[L] / 2, y1 + border[B] / 2, x0,
                              y1, x1, y1, x1 + border[R] / 2, y1 + border[B] / 2);
                    break;
                case BS_DOUBLE:
                    draw_quad(ctx, dev, ctm, style->border_color[B], x0 - border[L], y1 + border[B],
                              x0 - 2 * border[L] / 3, y1 + 2 * border[B] / 3, x1 + 2 * border[R] / 3,
                              y1 + 2 * border[B] / 3, x1 + border[R], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, style->border_color[B], x0 - border[L] / 3, y1 + border[B] / 3, x0, y1, x1,
                              y1, x1 + border[R] / 3, y1 + border[B] / 3);
                    break;
                case BS_DOTTED: {
                    float left = border[L] - border[B] / 2;
                    float right = border[R] - border[B] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[B], 0, border[B], x1 + right,
                                     y1 + border[B] / 2, x0 - left, y1 + border[B] / 2);
                    break;
                }
                case BS_DASHED: {
                    float left = border[L] - border[B] / 2;
                    float right = border[R] - border[B] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[B], 1, border[B], x1 + right,
                                     y1 + border[B] / 2, x0 - left, y1 + border[B] / 2);
                    break;
                }
                case BS_SOLID:
        ...
        case L:
            switch (collapse_border_style(style->border_style_3, collapsed)) {
                case BS_NONE:
                    return; /* Should never happen */
                case BS_OUTSET:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[L]), x0 - border[L], y0 - border[T], x0, y0,
                              x0, y1, x0 - border[L], y1 + border[B]);
                    break;
                case BS_INSET:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[L]), x0 - border[L], y0 - border[T], x0, y0, x0,
                              y1, x0 - border[L], y1 + border[B]);
                    break;
                case BS_RIDGE:
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[L]), x0 - border[L], y0 - border[T],
                              x0 - border[L] / 2, y0 - border[T] / 2, x0 - border[L] / 2, y1 + border[B] / 2,
                              x0 - border[L], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, darken(style->border_color[L]), x0 - border[L] / 2, y0 - border[T] / 2, x0,
                              y0, x0, y1, x0 - border[L] / 2, y1 + border[B] / 2);
                    break;
                case BS_GROOVE:
                    draw_quad(ctx, dev, ctm, darken(style->border_color[L]), x0 - border[L], y0 - border[T],
                              x0 - border[L] / 2, y0 - border[T] / 2, x0 - border[L] / 2, y1 + border[B] / 2,
                              x0 - border[L], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, brighten(style->border_color[L]), x0 - border[L] / 2, y0 - border[T] / 2,
                              x0, y0, x0, y1, x0 - border[L] / 2, y1 + border[B] / 2);
                    break;
                case BS_DOUBLE:
                    draw_quad(ctx, dev, ctm, style->border_color[L], x0 - border[L], y0 - border[T],
                              x0 - 2 * border[L] / 3, y0 - 2 * border[T] / 3, x0 - 2 * border[L] / 3,
                              y1 + 2 * border[B] / 3, x0 - border[L], y1 + border[B]);
                    draw_quad(ctx, dev, ctm, style->border_color[L], x0 - border[L] / 3, y0 - border[T] / 3, x0, y0, x0,
                              y1, x0 - border[L] / 3, y1 + border[B] / 3);
                    break;
                case BS_DOTTED: {
                    float top = border[T] - border[L] / 2;
                    float bottom = border[B] - border[L] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[L], 0, border[L], x0 - border[L] / 2,
                                     y1 + bottom, x0 - border[L] / 2, y0 - top);
                    break;
                }
                case BS_DASHED: {
                    float top = border[T] - border[L] / 2;
                    float bottom = border[B] - border[L] / 2;
                    draw_dotted_line(ctx, dev, ctm, style->border_color[L], 1, border[L], x0 - border[L] / 2,
                                     y1 + bottom, x0 - border[L] / 2, y0 - top);
                    break;
                }
                case BS_SOLID:
...
static int draw_block_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot, fz_device* dev,
    ...
    if (restart) {
        if (restart->start == box) restart->start = NULL;
        if (restart->end == box) return 1;
    }

    if (restart && restart->end == box) return 1;
    ...
    if (box->style->visibility == V_VISIBLE) {
        ...
        if (y1 > y0) {
            float bg_x0 = x0 - cell_adjust_left;
...
void fz_draw_restarted_html(fz_context* ctx, fz_device* dev, fz_matrix ctm, fz_html_box* top, float page_top,
                            float page_bot, fz_html_restarter* restart) {
    fz_html_box* box;
    ...
    fz_try(ctx) {
        ...
        for (box = top->down; box; box = box->next)
            if (draw_box(ctx, box, page_top, page_bot, dev, ctm, hb_buf, restart)) break;
    }
    fz_always(ctx) {
        if (unlocked) fz_hb_lock(ctx);
...
void fz_draw_story(fz_context* ctx, fz_story* story, fz_device* dev, fz_matrix ctm) {
    ...
    if (story == NULL || story->complete) return;
    ...
    if (dev) {
        ...
        fz_always(ctx) fz_drop_path(ctx, clip);
        fz_catch(ctx) fz_rethrow(ctx);
    }
    ...
    if (story->restart_place.start == NULL) story->complete = 1;

    if (dev) fz_pop_clip(ctx, dev);
}
...
static char* gather_text(fz_context* ctx, fz_html_box* box) {
    fz_html_flow* node;
    char* text = NULL;
...
static int enumerate_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot,
                         fz_story_position_callback* cb, void* arg, int depth, int rect_num,
                         fz_html_restarter* restart);

static int enumerate_block_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot,
                               fz_story_position_callback* cb, void* arg, int depth, int rect_num,
                               fz_html_restarter* restart) {
    fz_html_box* child;
    float y0, y1;
    ...
    if (box->style->visibility == V_VISIBLE && !skipping) {
        ...
        if (heading || box->id != NULL || box->href) {
            ...
            char* text = NULL;
            pos.text = NULL;
            if (heading && box->down) pos.text = text = gather_text(ctx, box->down);
            pos.depth = depth;
            pos.heading = heading;
            pos.open_close = 1;
            pos.id = box->id;
            ...
            fz_try(ctx) cb(ctx, arg, &pos);
            fz_always(ctx) fz_free(ctx, text);
            fz_catch(ctx) fz_rethrow(ctx);
            pos.text = NULL;
        }
    }
...
static int enumerate_flow_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot,
                              fz_story_position_callback* cb, void* arg, int depth, int rect_num,
                              fz_html_restarter* restart) {
    fz_html_flow* node;
    int restartable_ended = 0;
...
static int enumerate_box(fz_context* ctx, fz_html_box* box, float page_top, float page_bot,
    ...
    switch (box->type) {
        ...
        case BOX_BLOCK:
            if (restart && restart->end == box) return 1;
            if (enumerate_block_box(ctx, box, page_top, page_bot, cb, arg, depth, rect_num, restart)) return 1;
            break;
        case BOX_FLOW:
            if (enumerate_flow_box(ctx, box, page_top, page_bot, cb, arg, depth, rect_num, restart)) return 1;
            break;
    }
...
void fz_story_positions(fz_context* ctx, fz_story* story, fz_story_position_callback* cb, void* arg) {
    ...
    for (box = top->down; box; box = box->next)
        if (enumerate_box(ctx, box, page_top, page_bot, cb, arg, 0, story->rect_count + 1, &restart)) break;
}
