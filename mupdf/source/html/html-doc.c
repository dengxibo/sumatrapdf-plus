...
static fz_link_dest htdoc_resolve_link(fz_context* ctx, fz_document* doc_, const char* dest) {
    html_document* doc = (html_document*)doc_;
...
static int htdoc_count_pages(fz_context* ctx, fz_document* doc_, int chapter) {
    html_document* doc = (html_document*)doc_;
    if (doc->html->tree.root->s.layout.b > 0) return ceilf(doc->html->tree.root->s.layout.b / doc->html->page_h);
    return 1;
}
...
void fz_htdoc_reparse_html(fz_context* ctx, fz_document* doc_, fz_buffer* buf, float w, float h, float em) {
    html_document* doc = (html_document*)doc_;
    fz_outline* outline = NULL;

    if (!doc || !doc->format || !buf) return;
    ...
    fz_try(ctx) {
        doc->html = fz_parse_html(ctx, doc->set, doc->zip, ".", buf, fz_user_css(ctx), doc->format->try_xml,
                                  doc->format->try_html5, doc->format->patch_mobi);
        fz_layout_html(ctx, doc->html, w, h, em);
        outline = fz_load_html_outline(ctx, doc->html);
        htdoc_update_outline(ctx, doc_, outline);
        fz_drop_outline(ctx, doc->outline);
        doc->outline = outline;
        outline = NULL;
    }
...
static fz_rect htdoc_bound_page(fz_context* ctx, fz_page* page_, fz_box_type box) {
...
static int htdoc_lookup_metadata(fz_context* ctx, fz_document* doc_, const char* key, char* buf, size_t size) {
    html_document* doc = (html_document*)doc_;
    if (!strcmp(key, FZ_META_FORMAT)) return 1 + (int)fz_strlcpy(buf, doc->format->format_name, size);
    if (!strcmp(key, FZ_META_INFO_TITLE) && doc->html->title) return 1 + (int)fz_strlcpy(buf, doc->html->title, size);
    return -1;
}

static fz_html* generic_parse(fz_context* ctx, fz_html_font_set* set, fz_archive* zip, const char* base_uri,
                              fz_buffer* buffer_in, const char* user_css, const fz_htdoc_format_t* format) {
    ...
    fz_try(ctx) {
        ...
        html = fz_parse_html(ctx, set, zip, base_uri, buffer_html, user_css, format->try_xml, format->try_html5,
                             format->patch_mobi);
    }
    fz_always(ctx) {
...
fz_document* fz_htdoc_open_document_with_buffer(fz_context* ctx, fz_archive* dir, fz_buffer* buf,
                                                const fz_htdoc_format_t* format) {
...
fz_document* fz_htdoc_open_document_with_stream_and_dir(fz_context* ctx, fz_stream* stm, fz_archive* dir,
                                                        const fz_htdoc_format_t* format) {
    fz_buffer* buf = NULL;

    if (stm) buf = fz_read_all(ctx, stm, 0);
...
static int recognize_html_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
                                  fz_archive* dir, void** hstate, fz_document_recognize_state_free_fn** free_state,
                                  int xhtml) {
    uint8_t buffer[4096];
    size_t i, n, m;
    enum {
...
int htdoc_recognize_html_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
                                 fz_archive* dir, void** hstate, fz_document_recognize_state_free_fn** free_state) {
    return recognize_html_content(ctx, handler, stream, dir, hstate, free_state, 0);
}

static const fz_htdoc_format_t fz_htdoc_html5 = {"HTML5", NULL, 0, 1, 0};

static fz_document* htdoc_open_document(fz_context* ctx, const fz_document_handler* handler, fz_stream* file,
...
fz_document_handler html_document_handler = {
    NULL, htdoc_open_document, htdoc_extensions, htdoc_mimetypes, htdoc_recognize_html_content, 1};

/* XHTML document handler */

static const fz_htdoc_format_t fz_htdoc_xhtml = {"XHTML", NULL, 1, 1, 0};

static fz_document* xhtdoc_open_document(fz_context* ctx, const fz_document_handler* handler, fz_stream* file,
                                         fz_stream* accel, fz_archive* dir, void* state) {
    return fz_htdoc_open_document_with_stream_and_dir(ctx, file, dir, &fz_htdoc_xhtml);
}

int xhtdoc_recognize_xhtml_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
                                   fz_archive* dir, void** hstate, fz_document_recognize_state_free_fn** free_state) {
...
fz_document_handler xhtml_document_handler = {
    NULL, xhtdoc_open_document, xhtdoc_extensions, xhtdoc_mimetypes, xhtdoc_recognize_xhtml_content, 1};

/* FB2 document handler */

static const fz_htdoc_format_t fz_htdoc_fb2 = {"FictionBook2", NULL, 1, 0, 0};

static fz_document* fb2doc_open_document(fz_context* ctx, const fz_document_handler* handler, fz_stream* file,
                                         fz_stream* accel, fz_archive* dir, void* state) {
    return fz_htdoc_open_document_with_stream_and_dir(ctx, file, dir, &fz_htdoc_fb2);
}

static int fb2doc_recognize_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
...
fz_document_handler fb2_document_handler = {NULL, fb2doc_open_document, fb2doc_extensions, fb2doc_mimetypes,
                                            fb2doc_recognize_content};

/* Mobi document handler */

static const fz_htdoc_format_t fz_htdoc_mobi = {"MOBI", NULL, 1, 1, 1};

static fz_document* mobi_open_document_with_buffer(fz_context* ctx, fz_buffer* mobi) {
    fz_archive* dir = NULL;
    fz_buffer* html;
    fz_document* doc;
    fz_var(dir);
...
static int mobi_recognize_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
                                  fz_archive* dir, void** state, fz_document_recognize_state_free_fn** free_state) {
    char text[8];
    ...
    if (fz_read(ctx, stream, (unsigned char*)text, 8) != 8) return 0;
    if (memcmp(text, "BOOKMOBI", 8) == 0) return 100;
    if (memcmp(text, "TEXtREAd", 8) == 0) return 100;

    return 0;
}

static fz_document* mobi_open_document(fz_context* ctx, const fz_document_handler* handler, fz_stream* file,
                                       fz_stream* accel, fz_archive* dir, void* state) {
    return mobi_open_document_with_buffer(ctx, fz_read_all(ctx, file, 0));
}

static const char* mobi_extensions[] = {"mobi", "prc", "pdb", NULL};

static const char* mobi_mimetypes[] = {"application/x-mobipocket-ebook", NULL};

fz_document_handler mobi_document_handler = {NULL, mobi_open_document, mobi_extensions, mobi_mimetypes,
