...
/* SumatraPDF Plus: reparse a single-chapter HTML/MD document with a new buffer
 * and CSS already applied via fz_style_document. */
void
fz_htdoc_reparse_html(fz_context *ctx, fz_document *doc_, fz_buffer *buf, float w, float h, float em)
{
	html_document *doc = (html_document*)doc_;
	fz_outline *outline = NULL;

	if (!doc || !doc->format || !buf)
		return;
	...
	fz_try(ctx)
	{
		doc->html = fz_parse_html(ctx, doc->set, doc->zip, ".", buf, doc->super.user_css,
			doc->format->try_xml, doc->format->try_html5, doc->format->flavor, doc->super.publisher_css);
		fz_layout_html(ctx, doc->html, w, h, em);
		doc->super.layout_w = w;
		doc->super.layout_h = h;
		doc->super.layout_em = em;
