...
/* SumatraPDF Plus: drop the EPUB HTML font set so a reader font-family change
 * reloads fonts. Adapted for MuPDF 1.28 per-document layout/CSS fields. */
void
fz_reset_epub_html_font_set(fz_context *ctx, fz_document *doc_)
{
	epub_document *doc;
	char format[16];
	...
	if (doc->accel)
		invalidate_accelerator(ctx, doc->accel);
	doc->super.layout_w = 0;
	doc->super.layout_h = 0;
	doc->super.layout_em = 0;
	fz_drop_html_font_set(ctx, doc->set);
	doc->set = fz_new_html_font_set(ctx);
	doc->user_css_sum = checksum_css(ctx, doc->super.user_css);
}
