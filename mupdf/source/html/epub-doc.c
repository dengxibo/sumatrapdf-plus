...
static uint32_t user_css_sum(fz_context* ctx) {
    uint32_t sum = 0;
    const char* css = fz_user_css(ctx);
    sum = crc32(0, NULL, 0);
    if (css) sum = crc32(sum, (Byte*)css, (int)strlen(css));
    return sum;
}
...
static fz_stream* open_encrypted_entry(fz_context* ctx, fz_archive* arch_, const char* name) {
    struct encrypted* arch = (struct encrypted*)arch_;
    if (fz_tree_lookup(ctx, arch->info, name)) return NULL;
...
static void epub_parse_encryption(fz_context* ctx, epub_document* doc, fz_xml* root) {
    fz_tree* info = NULL;
    fz_xml* edata;
...
static int count_laid_out_pages(fz_html* html) {
    if (html->tree.root->s.layout.b > 0) return ceilf(html->tree.root->s.layout.b / html->page_h);
    return 1;
}

static void invalidate_accelerator(fz_context* ctx, epub_accelerator* acc) {
    int i;

    for (i = 0; i < acc->max_chapters; i++) acc->pages_in_chapter[i] = -1;
}
...
static fz_link_dest epub_resolve_link(fz_context* ctx, fz_document* doc_, const char* dest) {
    ...
    for (i = 0, ch = doc->spine; ch; ++i, ch = ch->next) {
        if (!strncmp(ch->path, dest, n) && ch->path[n] == 0) {
            if (s) {
                ...
                /* Broken NCX/nav fragment (common in repackaged Chinese EPUBs):
                   fall back to the start of the spine chapter instead of failing. */
...
static void epub_layout(fz_context* ctx, fz_document* doc_, float w, float h, float em) {
    epub_document* doc = (epub_document*)doc_;
    uint32_t css_sum = user_css_sum(ctx);
    int use_doc_css = fz_use_document_css(ctx);

    if (doc->layout_w == w && doc->layout_h == h && doc->layout_em == em && doc->css_sum == css_sum) return;
    doc->layout_w = w;
    doc->layout_h = h;
    doc->layout_em = em;

    if (doc->accel == NULL) return;
    ...
    if (doc->accel->layout_w == w && doc->accel->layout_h == h && doc->accel->layout_em == em &&
        doc->accel->use_doc_css == use_doc_css && doc->accel->css_sum == css_sum)
        return;
...
static void epub_build_chapters_cached(epub_document* doc, fz_context* ctx) {
    epub_chapter* ch;
    int n = 0;
    for (ch = doc->spine; ch; ch = ch->next) n++;
    fz_free(ctx, doc->chapters_cached);
    doc->chapters_cached = fz_malloc_array(ctx, n, epub_chapter*);
    doc->chapters_cached_len = n;
    int i = 0;
    for (ch = doc->spine; ch; ch = ch->next) doc->chapters_cached[i++] = ch;
}

static int epub_count_chapters(fz_context* ctx, fz_document* doc_) {
    epub_document* doc = (epub_document*)doc_;
    if (doc->chapters_cached_len == 0) epub_build_chapters_cached(doc, ctx);
    return doc->chapters_cached_len;
}

static int epub_count_pages(fz_context* ctx, fz_document* doc_, int chapter) {
    epub_document* doc = (epub_document*)doc_;
    if (chapter < 0) return 0;
    if (chapter >= doc->chapters_cached_len) epub_build_chapters_cached(doc, ctx);
    if (chapter >= doc->chapters_cached_len) return 0;
...
static void epub_load_accelerator(fz_context* ctx, epub_document* doc, fz_stream* accel) {
    ...
    if (accel) {
        ...
        fz_try(ctx) {
            ...
            for (v = 0; v < num_chapters; v++) acc->pages_in_chapter[v] = fz_read_int32_le(ctx, accel);
        }
        fz_catch(ctx) {
            if (acc) fz_free(ctx, acc->pages_in_chapter);
            fz_free(ctx, acc);
...
static void accelerate_chapter(fz_context* ctx, epub_document* doc, epub_chapter* ch, fz_html* html) {
    epub_accelerator* acc = doc->accel;
    ...
    if (ch->number >= acc->max_chapters) {
        ...
        int i;
        if (n == 0) n = 4;
        while (n <= ch->number) n *= 2;

        acc->pages_in_chapter = fz_realloc_array(ctx, acc->pages_in_chapter, n, int);
        for (i = acc->max_chapters; i < n; i++) acc->pages_in_chapter[i] = -1;
        acc->max_chapters = n;
    }
    acc->pages_in_chapter[ch->number] = p;
    if (acc->num_chapters < ch->number + 1) acc->num_chapters = ch->number + 1;
}

static void epub_drop_page(fz_context* ctx, fz_page* page_) {
    epub_page* page = (epub_page*)page_;
    fz_drop_html(ctx, page->html);
}

static epub_chapter* epub_load_chapter(fz_context* ctx, epub_document* doc, const char* path, int i) {
    epub_chapter* ch;
...
static fz_html* epub_parse_chapter(fz_context* ctx, epub_document* doc, epub_chapter* ch) {
    ...
    if (!buf)
        buf = fz_new_buffer_from_printf(ctx, "<html><body><p><i>ERROR: cannot find chapter %<</i></p></body></html>",
                                        ch->path);
    fz_try(ctx) html = fz_parse_html(ctx, doc->set, zip, base_uri, buf, fz_user_css(ctx), 1, 1, 0);
...
static fz_rect epub_bound_page(fz_context* ctx, fz_page* page_, fz_box_type box) {
...
static void epub_run_page(fz_context* ctx, fz_page* page_, fz_device* dev, fz_matrix ctm, fz_cookie* cookie) {
    epub_page* page = (epub_page*)page_;

    fz_draw_html(ctx, dev, ctm, page->html, page->number);
}

static fz_link* epub_load_links(fz_context* ctx, fz_page* page_) {
    epub_page* page = (epub_page*)page_;
    epub_chapter* ch = page->ch;
...
static fz_bookmark epub_make_bookmark(fz_context* ctx, fz_document* doc_, fz_location loc) {
    epub_document* doc = (epub_document*)doc_;
    epub_chapter* ch;
    int i;
...
static fz_location epub_lookup_bookmark(fz_context* ctx, fz_document* doc_, fz_bookmark mark) {
    epub_document* doc = (epub_document*)doc_;
    epub_chapter* ch;
    int i;
    ...
    for (i = 0, ch = doc->spine; ch; ++i, ch = ch->next) {
        ...
        fz_drop_html(ctx, html);
        if (p != -1) return fz_make_location(i, p);
    }
    return fz_make_location(-1, -1);
}

static fz_page* epub_load_page(fz_context* ctx, fz_document* doc_, int chapter, int number) {
    epub_document* doc = (epub_document*)doc_;
    epub_chapter* ch;
    int i;

    if (chapter < 0) fz_throw(ctx, FZ_ERROR_ARGUMENT, "invalid chapter number: %d", chapter);
    if (number < 0) fz_throw(ctx, FZ_ERROR_ARGUMENT, "invalid page number: %d", number);
...
static void epub_page_label(fz_context* ctx, fz_document* doc_, int chapter, int number, char* buf, size_t size) {
    fz_snprintf(buf, size, "ch. %d, p. %d", chapter + 1, number + 1);
}

static void epub_drop_accelerator(fz_context* ctx, epub_accelerator* acc) {
    if (acc == NULL) return;
...
static void epub_drop_document(fz_context* ctx, fz_document* doc_) {
    epub_document* doc = (epub_document*)doc_;
...
static const char* rel_path_from_idref(fz_xml* manifest, const char* idref) {
    fz_xml* item;
    if (!idref) return NULL;
    item = fz_xml_find_down(manifest, "item");
    while (item) {
        const char* id = fz_xml_att(item, "id");
        if (id && !strcmp(id, idref)) return fz_xml_att(item, "href");
        item = fz_xml_find_next(item, "item");
    }
    return NULL;
}

static const char* path_from_idref(char* path, fz_xml* manifest, const char* base_uri, const char* idref, int n) {
...
static const char* rel_path_from_prop(fz_xml* manifest, const char* prop) {
    fz_xml* item;
    if (!prop) return NULL;
    item = fz_xml_find_down(manifest, "item");
    while (item) {
        const char* id = fz_xml_att(item, "properties");
        if (id && !strcmp(id, prop)) return fz_xml_att(item, "href");
        item = fz_xml_find_next(item, "item");
    }
    return NULL;
}

static const char* path_from_prop(char* path, fz_xml* manifest, const char* base_uri, const char* prop, int n) {
...
static fz_outline* epub_parse_ncx_imp(fz_context* ctx, epub_document* doc, fz_xml* node, char* base_uri) {
    char path[2048];
...
static void epub_parse_ncx(fz_context* ctx, epub_document* doc, const char* path) {
    fz_archive* zip = doc->zip;
    fz_buffer* buf = NULL;
    fz_xml_doc* ncx = NULL;
    char base_uri[2048];
...
static fz_outline* epub_parse_nav_imp(fz_context* ctx, epub_document* doc, fz_xml* ol, char* base_uri) {
    ...
    fz_try(ctx) {
        ...
        while (li) {
            ...
            *tailp = outline = fz_new_outline(ctx);
            ...
            if (fz_xml_is_tag(li_ol, "ol")) outline->down = epub_parse_nav_imp(ctx, doc, li_ol, base_uri);
...
static void epub_parse_nav(fz_context* ctx, epub_document* doc, const char* path) {
    fz_archive* zip = doc->zip;
    fz_buffer* buf = NULL;
    ...
    fz_try(ctx) {
        ...
        if (nav) doc->outline = epub_parse_nav_imp(ctx, doc, fz_xml_find_down(nav, "ol"), base_uri);
    }
    fz_always(ctx) {
        fz_drop_buffer(ctx, buf);
...
static char* find_metadata(fz_context* ctx, fz_xml* metadata, char* key) {
    char* text = fz_xml_text(fz_xml_down(fz_xml_find_down(metadata, key)));
    if (text) return fz_strdup(ctx, text);
    return NULL;
}

static fz_buffer* read_container_and_prefix(fz_context* ctx, fz_archive* zip, char* prefix, size_t prefix_len) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!strcmp(p, "META-INF/container.xml")) return fz_read_archive_entry(ctx, zip, "META-INF/container.xml");
    }
...
static void epub_parse_header(fz_context* ctx, epub_document* doc) {
    fz_archive* zip = doc->zip;
    fz_buffer* buf = NULL;
    ...
    fz_try(ctx) {
        ...
        prefix_len = strlen(base_uri);
        {
            ...
            if (sizeof(base_uri) <= prefix_len + z0) fz_throw(ctx, FZ_ERROR_FORMAT, "Prefix too long in epub");
        ...
        if (!full_path) fz_throw(ctx, FZ_ERROR_FORMAT, "cannot find root file in EPUB");
        ...
        if (!version || strcmp(version, "2.0")) fz_warn(ctx, "unknown epub version: %s", version ? version : "<none>");
...
static fz_outline* epub_load_outline(fz_context* ctx, fz_document* doc_) {
    epub_document* doc = (epub_document*)doc_;
    return fz_keep_outline(ctx, doc->outline);
}

static int epub_lookup_metadata(fz_context* ctx, fz_document* doc_, const char* key, char* buf, size_t size) {
    epub_document* doc = (epub_document*)doc_;
    if (!strcmp(key, FZ_META_FORMAT)) return 1 + (int)fz_strlcpy(buf, "EPUB", size);
    if (!strcmp(key, FZ_META_INFO_TITLE) && doc->dc_title) return 1 + (int)fz_strlcpy(buf, doc->dc_title, size);
    if (!strcmp(key, FZ_META_INFO_AUTHOR) && doc->dc_creator) return 1 + (int)fz_strlcpy(buf, doc->dc_creator, size);
    return -1;
}

static void epub_output_accelerator(fz_context* ctx, fz_document* doc_, fz_output* out) {
    epub_document* doc = (epub_document*)doc_;
    int i;

    fz_try(ctx) {
        if (doc->accel == NULL) fz_throw(ctx, FZ_ERROR_ARGUMENT, "No accelerator data to write");
        ...
        for (i = 0; i < doc->accel->num_chapters; i++) fz_write_int32_le(ctx, out, doc->accel->pages_in_chapter[i]);
...
static fz_document* epub_open_document(fz_context* ctx, const fz_document_handler* handler, fz_stream* file,
                                       fz_stream* accel, fz_archive* dir, void* state) {
    ...
    if (file == NULL) {
        ...
        if (fz_has_archive_entry(ctx, dir, "META-INF/container.xml"))
            ...
            file2 = file = fz_open_archive_entry(ctx, dir, "META-INF\\container.xml");
        if (file == NULL) fz_throw(ctx, FZ_ERROR_FORMAT, "Not an epub file");
    ...
    fz_try(ctx) doc = epub_init(ctx, zip, accel);
    fz_always(ctx) fz_drop_stream(ctx, file2);
    fz_catch(ctx) fz_rethrow(ctx);

    return doc;
}

static int epub_recognize(fz_context* doc, const fz_document_handler* handler, const char* magic) {
    if (strstr(magic, "META-INF/container.xml") || strstr(magic, "META-INF\\container.xml")) return 200;
    return 0;
}

static int epub_recognize_content(fz_context* ctx, const fz_document_handler* handler, fz_stream* stream,
                                  fz_archive* dir, void** state, fz_document_recognize_state_free_fn** free_state) {
...
fz_document_handler epub_document_handler = {epub_recognize, epub_open_document, epub_extensions, epub_mimetypes,
