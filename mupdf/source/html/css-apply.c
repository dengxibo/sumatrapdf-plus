...
static const char* list_style_type_kw[] = {
    "armenian",    "circle",      "decimal",     "decimal-leading-zero", "disc", "georgian",
...
static const char* font_family_ignore[] = {
    ",",       "/",     "bold",       "bolder",  "italic",  "large",   "larger",   "lighter",  "medium",
    "oblique", "small", "small-caps", "smaller", "x-large", "x-small", "xx-large", "xx-small",
};

static int keyword_in_list(const char* name, const char** list, int n) {
    int l = 0;
    int r = n - 1;
    while (l <= r) {
        int m = (l + r) >> 1;
...
static inline int fz_tolower_ascii(int c) {
    if ((unsigned int)c - 'A' < 26) return c | 32;
    return c;
}

static int fz_strcasecmp_ascii(const char* a, const char* b) {
    const unsigned char *l = (void*)a, *r = (void*)b;
    while (*l && *r && (*l == *r || fz_tolower_ascii(*l) == fz_tolower_ascii(*r))) l++, r++;
    return fz_tolower_ascii(*l) - fz_tolower_ascii(*r);
}

static const fz_css_color* lookup_named_color(const char* name) {
    int l = 0;
...
static int count_selector_ids(fz_css_selector* sel) {
...
static int count_condition_atts(fz_css_condition* cond) {
    int n = 0;
    while (cond) {
        if (cond->type != '#' && cond->type != ':') n++;
        cond = cond->next;
    }
    return n;
}

static int count_selector_atts(fz_css_selector* sel) {
...
static int count_condition_names(fz_css_condition* cond) {
    int n = 0;
    while (cond) {
        if (cond->type == ':') n++;
        cond = cond->next;
...
static int count_selector_names(fz_css_selector* sel) {
...
static int selector_specificity(fz_css_selector* sel, int important) {
...
static int match_att_is_condition(fz_xml* node, const char* key, const char* val) {
    const char* att = fz_xml_att(node, key);
    return att && !strcmp(val, att);
}

static int match_att_has_condition(fz_xml* node, const char* att, const char* needle) {
    ...
    if (haystack) {
        ...
        if (ss) {
            ...
            if ((ss[n] == ' ' || ss[n] == 0) && (ss == haystack || ss[-1] == ' ')) return 1;
        }
    }
    return 0;
}

static int match_nth(int count, int a, int b) {
    if (a == 0) {
        return count == b;
    } else {
        /* (count - b) / a is positive or (count - b) is 0 */
        int delta = count - b;
        if (((delta > 0) == (a > 0)) || delta == 0) /* (count - b) / a is integer */
            return (delta % a == 0);
        return 0;
    }
}

static int count_siblings_before(fz_xml* target) {
    fz_xml* node;
    int count = 0;
...
static int count_siblings_of_type_before(fz_xml* target) {
...
static int match_nth_child(fz_xml* target, int a, int b) {
...
static int match_an_plus_b_microsyntax(fz_xml* node, const char* val, int (*callback)(fz_xml* node, int a, int b)) {
    ...
    if (sscanf(val, " even %n", &n) == 0 && n == m) return callback(node, 2, 0);
    if (sscanf(val, " odd %n", &n) == 0 && n == m) return callback(node, 2, 1);

    if (sscanf(val, " %dn + %d %n", &a, &b, &n) == 2 && n == m) return callback(node, a, b);
    if (sscanf(val, " %dn - %d %n", &a, &b, &n) == 2 && n == m) return callback(node, a, -b);
    if (sscanf(val, " %dn %n", &a, &n) == 1 && n == m) return callback(node, a, 0);
    if (sscanf(val, " %d %n", &b, &n) == 1 && n == m) return callback(node, 0, b);

    if (sscanf(val, " -n + %d %n", &b, &n) == 1 && n == m) return callback(node, -1, b);
    if (sscanf(val, " -n - %d %n", &b, &n) == 1 && n == m) return callback(node, -1, -b);

    if (sscanf(val, " n + %d %n", &b, &n) == 1 && n == m) return callback(node, 1, b);
    if (sscanf(val, " n - %d %n", &b, &n) == 1 && n == m) return callback(node, 1, -b);

    if (sscanf(val, " n %n", &n) == 0 && n == m) return 1;

    return 0;
}

static int match_pseudo_condition(fz_xml* node, const char* key, const char* val) {
    if (!strcmp(key, "link")) return fz_xml_att(node, "href") != NULL;

    if (!strcmp(key, "empty")) return fz_xml_down(node) == NULL;
...
static int match_condition(fz_css_condition* cond, fz_xml* node) {
    ...
    switch (cond->type) {
        default:
            return 0;
        case ':':
            if (!match_pseudo_condition(node, cond->key, cond->val)) return 0;
            break;
        case '#':
            if (!match_att_is_condition(node, "id", cond->val)) return 0;
            break;
        case '.':
            if (!match_att_has_condition(node, "class", cond->val)) return 0;
            break;
        case '[':
            if (!match_att_exists_condition(node, cond->key)) return 0;
            break;
        case '=':
            if (!match_att_is_condition(node, cond->key, cond->val)) return 0;
            break;
        case '~':
            if (!match_att_has_condition(node, cond->key, cond->val)) return 0;
            break;
        case '|':
            if (!match_att_is_condition(node, cond->key, cond->val)) return 0;
            break;
...
static int match_selector(fz_css_selector* sel, fz_xml* node) {
    ...
    if (sel->combine) {
        ...
        if (sel->combine == ' ') {
            ...
            if (!parent || !match_selector(sel->right, node)) return 0;

            while (parent) {
                if (match_selector(sel->left, parent)) return 1;
        ...
        if (sel->combine == '>') {
            ...
            if (!match_selector(sel->right, node)) return 0;
        }

        /* adjacent */
        if (sel->combine == '+') {
            fz_xml* prev = fz_xml_prev(node);
            while (prev && !fz_xml_tag(prev)) prev = fz_xml_prev(prev);
            if (!prev) return 0;
    ...
    if (sel->name) {
        if (!fz_xml_is_tag(node, sel->name)) return 0;
    }

    if (sel->cond) {
        if (!match_condition(sel->cond, node)) return 0;
    }
...
static void add_shorthand_trbl(fz_css_match* match, fz_css_value* value, int spec, int name_t, int name_r, int name_b,
                               int name_l) {
...
static void add_shorthand_inset(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_INSET_TOP, PRO_INSET_RIGHT, PRO_INSET_BOTTOM, PRO_INSET_LEFT);
}

static void add_shorthand_margin(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_MARGIN_TOP, PRO_MARGIN_RIGHT, PRO_MARGIN_BOTTOM, PRO_MARGIN_LEFT);
}

static void add_shorthand_padding(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_PADDING_TOP, PRO_PADDING_RIGHT, PRO_PADDING_BOTTOM, PRO_PADDING_LEFT);
}

static void add_shorthand_border_width(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_BORDER_TOP_WIDTH, PRO_BORDER_RIGHT_WIDTH, PRO_BORDER_BOTTOM_WIDTH,
                       PRO_BORDER_LEFT_WIDTH);
}

static void add_shorthand_border_color(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_BORDER_TOP_COLOR, PRO_BORDER_RIGHT_COLOR, PRO_BORDER_BOTTOM_COLOR,
                       PRO_BORDER_LEFT_COLOR);
}

static void add_shorthand_border_style(fz_css_match* match, fz_css_value* value, int spec) {
    add_shorthand_trbl(match, value, spec, PRO_BORDER_TOP_STYLE, PRO_BORDER_RIGHT_STYLE, PRO_BORDER_BOTTOM_STYLE,
                       PRO_BORDER_LEFT_STYLE);
}

static void add_shorthand_border(fz_css_match* match, fz_css_value* value, int spec, int T, int R, int B, int L) {
    while (value) {
...
static void add_shorthand_list_style(fz_css_match* match, fz_css_value* value, int spec) {
    while (value) {
...
static fz_css_value* add_shorthand_font_size(fz_css_match* match, fz_css_value* value, int spec) {
    /* font-size */
...
static void add_shorthand_font(fz_css_match* match, fz_css_value* value, int spec) {
...
static void add_shorthand_background(fz_css_match* match, fz_css_value* value, int spec) {
...
void fz_add_css_font_face(fz_context* ctx, fz_html_font_set* set, fz_archive* zip, const char* base_uri,
    ...
    for (custom = set->custom; custom; custom = custom->next)
        if (!strcmp(custom->src, path) && !strcmp(custom->family, family) && custom->is_bold == is_bold &&
    ...
    fz_try(ctx) {
        if (fz_has_archive_entry(ctx, zip, path))
            ...
            if (stm == NULL) fz_throw(ctx, FZ_ERROR_FORMAT, "cannot locate font '%s' specified by css", src);
...
static int is_inheritable_property(int name) {
    return name == PRO_COLOR || name == PRO_DIRECTION || name == PRO_FONT_FAMILY || name == PRO_FONT_STYLE ||
           name == PRO_FONT_VARIANT || name == PRO_FONT_WEIGHT || name == PRO_LEADING || name == PRO_LETTER_SPACING ||
           name == PRO_LINE_HEIGHT || name == PRO_LIST_STYLE_IMAGE || name == PRO_LIST_STYLE_POSITION ||
           name == PRO_LIST_STYLE_TYPE || name == PRO_ORPHANS || name == PRO_OVERFLOW_WRAP || name == PRO_QUOTES ||
           name == PRO_TEXT_ALIGN || name == PRO_TEXT_INDENT || name == PRO_TEXT_TRANSFORM || name == PRO_VISIBILITY ||
           name == PRO_WHITE_SPACE || name == PRO_WIDOWS || name == PRO_WORD_SPACING || name == PRO_HYPHENS ||
...
static fz_css_value* value_from_inheritable_property(fz_css_match* match, int name) {
    while (match) {
        fz_css_value* value = match->value[name];
        if (value && strcmp(value->data, "inherit") != 0) return value;
        match = match->up;
    }
    return NULL;
}

static fz_css_value* value_from_property(fz_css_match* match, int name) {
    fz_css_value* value = match->value[name];
    if (match->up) {
        ...
        if (!value && is_inheritable_property(name)) return value_from_inheritable_property(match->up, name);
    }
    return value;
}

static const char* string_from_property(fz_css_match* match, int name, const char* initial) {
    fz_css_value* value = value_from_property(match, name);
    if (!value) return initial;
    return value->data;
}

static fz_css_number make_number(float v, int u) {
    fz_css_number n;
    n.value = v;
    n.unit = u;
    return n;
...
static fz_css_number make_undefined_number(void) {
    fz_css_number n;
    n.value = 0;
    n.unit = N_UNDEFINED;
    return n;
...
static float fz_css_strtof(char* s, char** endptr) {
    float sign = 1;
    float v = 0;
    float n = 0;
    float d = 1;
...
static fz_css_number number_from_value(fz_css_value* value, float initial, int initial_unit) {
    ...
    if (!value) return make_number(initial, initial_unit);

    if (value->type == CSS_PERCENT) return make_number(fz_css_strtof(value->data, NULL), N_PERCENT);

    if (value->type == CSS_NUMBER) return make_number(fz_css_strtof(value->data, NULL), N_NUMBER);

    if (value->type == CSS_LENGTH) {
        float x = fz_css_strtof(value->data, &p);

        if (p[0] == 'e' && p[1] == 'm' && p[2] == 0) return make_number(x, N_SCALE);
        if (p[0] == 'e' && p[1] == 'x' && p[2] == 0) return make_number(x / 2, N_SCALE);

        if (p[0] == 'i' && p[1] == 'n' && p[2] == 0) return make_number(x * 72, N_LENGTH);
        if (p[0] == 'c' && p[1] == 'm' && p[2] == 0) return make_number(x * 7200 / 254, N_LENGTH);
        if (p[0] == 'm' && p[1] == 'm' && p[2] == 0) return make_number(x * 720 / 254, N_LENGTH);
        if (p[0] == 'p' && p[1] == 'c' && p[2] == 0) return make_number(x * 12, N_LENGTH);

        if (p[0] == 'p' && p[1] == 't' && p[2] == 0) return make_number(x, N_LENGTH);
        if (p[0] == 'p' && p[1] == 'x' && p[2] == 0) return make_number(x, N_LENGTH);

        /* FIXME: 'rem' should be 'em' of root element. This is a bad approximation. */
        if (p[0] == 'r' && p[1] == 'e' && p[2] == 'm' && p[3] == 0) return make_number(x * 16, N_LENGTH);

        /* FIXME: 'ch' should be width of '0' character. This is an approximation. */
        if (p[0] == 'c' && p[1] == 'h' && p[2] == 0) return make_number(x / 2, N_LENGTH);

        return make_number(x, N_LENGTH);
    }

    if (value->type == CSS_KEYWORD) {
        if (!strcmp(value->data, "auto")) return make_number(0, N_AUTO);
    }

    return make_number(initial, initial_unit);
}

static fz_css_number number_from_property(fz_css_match* match, int property, float initial, int initial_unit) {
    return number_from_value(value_from_property(match, property), initial, initial_unit);
}

static int has_number_from_property(fz_css_match* match, int property, float initial, int initial_unit,
                                    fz_css_number* num) {
...
static unsigned int has_border_width_from_property(fz_css_match* match, int property, fz_css_number* num) {
...
static fz_css_number border_width_from_property(fz_css_match* match, int property) {
    fz_css_number num;
...
static int border_style_from_property(fz_css_match* match, int property) {
    fz_css_value* value = value_from_property(match, property);
    if (value) {
        if (!strcmp(value->data, "none"))
            ...
            return BS_NONE;
        else if (!strcmp(value->data, "solid"))
            return BS_SOLID;
        else if (!strcmp(value->data, "dotted"))
            return BS_DOTTED;
        else if (!strcmp(value->data, "dashed"))
            return BS_DASHED;
        else if (!strcmp(value->data, "double"))
            return BS_DOUBLE;
        else if (!strcmp(value->data, "groove"))
            return BS_GROOVE;
        else if (!strcmp(value->data, "ridge"))
            return BS_RIDGE;
        else if (!strcmp(value->data, "inset"))
            return BS_INSET;
        else if (!strcmp(value->data, "outset"))
            return BS_OUTSET;
    }
    return BS_NONE;
}
...
float fz_from_css_number(fz_css_number number, float em, float percent_value, float auto_value) {
    switch (number.unit) {
        ...
        case N_PERCENT:
            return number.value * 0.01f * percent_value;
        case N_AUTO:
            return auto_value;
    }
}
...
float fz_from_css_number_scale(fz_css_number number, float scale) {
    switch (number.unit) {
        ...
        case N_SCALE:
            return number.value * scale;
        case N_PERCENT:
            return number.value * 0.01f * scale;
        case N_AUTO:
            return scale;
    }
}

static fz_css_color make_color(int r, int g, int b, int a) {
    fz_css_color c;
...
static size_t hexlen(const char* data) {
    size_t n = 0;

    while ((*data >= '0' && *data <= '9') || (*data >= 'a' && *data <= 'f') || (*data >= 'A' && *data <= 'F')) {
        data++, n++;
    }
...
static fz_css_color hash_color(const char* data) {
    int r, g, b, a;
...
static fz_css_color color_from_value(fz_css_value* value, fz_css_color initial) {
    if (!value) return initial;
...
fz_css_color fz_css_color_from_string(const char* str) {
    ...
    if (*str == '#') return hash_color(str + 1);
    named = lookup_named_color(str);
    if (named) return *named;
    return hash_color(str);
}

static fz_css_color color_from_property(fz_css_match* match, int property, fz_css_color initial) {
    return color_from_value(value_from_property(match, property), initial);
}

static unsigned int has_color_from_property(fz_css_match* match, int property, fz_css_color* col) {
...
static fz_css_color color_from_properties(fz_css_match* match, int property, int property2, fz_css_color initial) {
    fz_css_value* value = value_from_property(match, property);

    if (value == NULL) value = value_from_property(match, property2);
...
int fz_get_css_match_display(fz_css_match* match) {
    ...
    if (value) {
        if (!strcmp(value->data, "none")) return DIS_NONE;
        if (!strcmp(value->data, "inline")) return DIS_INLINE;
        if (!strcmp(value->data, "block")) return DIS_BLOCK;
        if (!strcmp(value->data, "list-item")) return DIS_LIST_ITEM;
        if (!strcmp(value->data, "inline-block")) return DIS_INLINE_BLOCK;
        if (!strcmp(value->data, "table")) return DIS_TABLE;
        if (!strcmp(value->data, "table-row")) return DIS_TABLE_ROW;
        if (!strcmp(value->data, "table-cell")) return DIS_TABLE_CELL;
        if (!strcmp(value->data, "table-row-group")) return DIS_TABLE_GROUP;
        if (!strcmp(value->data, "table-header-group")) return DIS_TABLE_GROUP;
        if (!strcmp(value->data, "table-footer-group")) return DIS_TABLE_GROUP;
        if (!strcmp(value->data, "table-column-group")) return DIS_TABLE_COLGROUP;
        if (!strcmp(value->data, "table-column")) return DIS_TABLE_COL;
        /* Not ideal, but it'll do. */
        if (!strcmp(value->data, "inline-table")) return DIS_TABLE;
    }
    return DIS_INLINE;
}

static int white_space_from_property(fz_css_match* match) {
    fz_css_value* value = value_from_property(match, PRO_WHITE_SPACE);
    if (value) {
        if (!strcmp(value->data, "normal"))
            return WS_NORMAL;
        else if (!strcmp(value->data, "pre"))
            return WS_PRE;
        else if (!strcmp(value->data, "nowrap"))
            return WS_NOWRAP;
        else if (!strcmp(value->data, "pre-wrap"))
            return WS_PRE_WRAP;
        else if (!strcmp(value->data, "pre-line"))
            return WS_PRE_LINE;
    }
    return WS_NORMAL;
}

static int text_decoration_from_property(fz_css_match* match) {
...
static unsigned int has_visibility_from_property(fz_css_match* match, unsigned int* val) {
...
static int visibility_from_property(fz_css_match* match) {
    unsigned int val;
...
static int page_break_from_property(fz_css_match* match, int prop) {
    fz_css_value* value = value_from_property(match, prop);
    if (value) {
        if (!strcmp(value->data, "auto"))
            return PB_AUTO;
        else if (!strcmp(value->data, "always"))
            return PB_ALWAYS;
        else if (!strcmp(value->data, "avoid"))
            return PB_AVOID;
        else if (!strcmp(value->data, "left"))
            return PB_LEFT;
        else if (!strcmp(value->data, "right"))
            return PB_RIGHT;
    }
    return PB_AUTO;
}
...
void fz_apply_css_style(fz_context* ctx, fz_html_font_set* set, fz_css_style* style, fz_css_match* match) {
    ...
    if (value) {
        ...
        else if (!strcmp(value->data, "center"))
            style->text_align = TA_CENTER;
        else if (!strcmp(value->data, "justify"))
    ...
    if (value) {
        ...
        else if (!strcmp(value->data, "fixed"))
            style->position = POS_FIXED;
        else if (!strcmp(value->data, "relative"))
            style->position = POS_RELATIVE;
        else if (!strcmp(value->data, "absolute"))
    ...
    if (value) {
        if (!strcmp(value->data, "baseline"))
            style->vertical_align = VA_BASELINE;
        else if (!strcmp(value->data, "sub"))
            style->vertical_align = VA_SUB;
        else if (!strcmp(value->data, "super"))
            style->vertical_align = VA_SUPER;
        else if (!strcmp(value->data, "top"))
            style->vertical_align = VA_TOP;
        else if (!strcmp(value->data, "bottom"))
            style->vertical_align = VA_BOTTOM;
        else if (!strcmp(value->data, "text-top"))
            style->vertical_align = VA_TEXT_TOP;
        else if (!strcmp(value->data, "text-bottom"))
            style->vertical_align = VA_TEXT_BOTTOM;
    }

    value = value_from_property(match, PRO_FONT_SIZE);
    if (value) {
        /* absolute-size */
        if (!strcmp(value->data, "xx-large"))
            style->font_size = make_number(1.73f, N_SCALE);
        else if (!strcmp(value->data, "x-large"))
            style->font_size = make_number(1.44f, N_SCALE);
        else if (!strcmp(value->data, "large"))
            style->font_size = make_number(1.2f, N_SCALE);
        else if (!strcmp(value->data, "medium"))
            style->font_size = make_number(1.0f, N_SCALE);
        else if (!strcmp(value->data, "small"))
            style->font_size = make_number(0.83f, N_SCALE);
        else if (!strcmp(value->data, "x-small"))
            style->font_size = make_number(0.69f, N_SCALE);
        else if (!strcmp(value->data, "xx-small"))
            style->font_size = make_number(0.69f, N_SCALE);
        /* relative-size */
        else if (!strcmp(value->data, "larger"))
            style->font_size = make_number(1.2f, N_SCALE);
        else if (!strcmp(value->data, "smaller"))
            style->font_size = make_number(1 / 1.2f, N_SCALE);
        /* percentage */
    ...
    if (value) {
        if (!strcmp(value->data, "none"))
            style->list_style_type = LST_NONE;
        else if (!strcmp(value->data, "disc"))
            style->list_style_type = LST_DISC;
        else if (!strcmp(value->data, "circle"))
            style->list_style_type = LST_CIRCLE;
        else if (!strcmp(value->data, "square"))
            style->list_style_type = LST_SQUARE;
        else if (!strcmp(value->data, "decimal"))
            style->list_style_type = LST_DECIMAL;
        else if (!strcmp(value->data, "decimal-leading-zero"))
            style->list_style_type = LST_DECIMAL_ZERO;
        else if (!strcmp(value->data, "lower-roman"))
            style->list_style_type = LST_LC_ROMAN;
        else if (!strcmp(value->data, "upper-roman"))
            style->list_style_type = LST_UC_ROMAN;
        else if (!strcmp(value->data, "lower-greek"))
            style->list_style_type = LST_LC_GREEK;
        else if (!strcmp(value->data, "upper-greek"))
            style->list_style_type = LST_UC_GREEK;
        else if (!strcmp(value->data, "lower-latin"))
            style->list_style_type = LST_LC_LATIN;
        else if (!strcmp(value->data, "upper-latin"))
            style->list_style_type = LST_UC_LATIN;
        else if (!strcmp(value->data, "lower-alpha"))
            style->list_style_type = LST_LC_ALPHA;
        else if (!strcmp(value->data, "upper-alpha"))
            style->list_style_type = LST_UC_ALPHA;
        else if (!strcmp(value->data, "armenian"))
            style->list_style_type = LST_ARMENIAN;
        else if (!strcmp(value->data, "georgian"))
            style->list_style_type = LST_GEORGIAN;
    }

    value = value_from_property(match, PRO_OVERFLOW_WRAP);
    if (value) {
        if (!strcmp(value->data, "break-word"))
            style->overflow_wrap = OVERFLOW_WRAP_BREAK_WORD;
        else
            style->overflow_wrap = OVERFLOW_WRAP_NORMAL;
    }

    value = value_from_property(match, PRO_BORDER_COLLAPSE);
    if (value) {
        if (!strcmp(value->data, "collapse"))
            style->border_collapse = 1;
        else
    ...
    if (value) {
        if (!strcmp(value->data, "none"))
            style->hyphens = HYP_NONE;
        else if (!strcmp(value->data, "manual"))
            style->hyphens = HYP_MANUAL;
        else if (!strcmp(value->data, "auto"))
            style->hyphens = HYP_AUTO;
    }
...

    {
        ...
        for (; value; value = value->next) {
            ...
            if (value->type == CSS_STRING ||
                (value->type == CSS_KEYWORD &&
                 !keyword_in_list(value->data, font_family_ignore, nelem(font_family_ignore)))) {
                style->font = fz_load_html_font(ctx, set, value->data, is_bold, is_italic, style->small_caps);
                if (style->font) break;
            }
        }
        if (!style->font) style->font = fz_load_html_font(ctx, set, "serif", is_bold, is_italic, style->small_caps);
    }
}

void fz_css_colstyle(col_style* cs, fz_css_match* match) {
...
static void do_verify_splay(const fz_css_style_splay* x) {
    printf("%x<", x);
    if (x->lt) {
...
static void verify_splay(const fz_css_style_splay* x) {
    if (x == NULL) return;
    assert(x->up == NULL);
    do_verify_splay(x);
    printf("-----\n");
...
const fz_css_style* fz_css_enlist(fz_context* ctx, const fz_css_style* style, fz_css_style_splay** tree,
                                  fz_pool* pool) {
...
static void print_property(fz_css_property* prop) {
    ...
    if (prop->important) printf(" !important");
    printf(";\n");
}
...
static void print_condition(fz_css_condition* cond) {
    ...
    if (cond->next) print_condition(cond->next);
}
